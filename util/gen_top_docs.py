...
Generates:
    <outdir>
    └── doc/memory_map.md
    └── ip_autogen/pinmux/doc/          # if there is a pinmux within top
        ├── pinout_<target>.md          # for each top-level target
        └── targets.md

Specifically, generate top-dependent pinmux and memory map docs, which are only fully known after
generating the complete top config. This step cannot be done while generating individual components
(such as instantiating the pinmux template) because top level information must be fully known.
"""
...
from topgen.lib import find_module, load_cfg, get_ip_hjson_path
...
def gen_pinmux_docs(top, c_helper, out_path):
    ...
    summary_table = f'# {topname.capitalize()} Pinout and Pinmux Connectivity Tables\n'
...
def gen_memory_map_docs(top, c_helper, out_path):
    ...
    template_path = Path(__file__).parent / "topgen" / "templates" / "memory_map.md.tpl"
    ...
    try:
        ...
        log.error(f"failed to write to {memory_map_path}")
        raise
...
def main():
    ...
    parser.add_argument("--outdir", "-o",
                        default=None,
                        type=Path,
                        help="Output top directory. Defaults to $(dirname completecfg)/../..")
    args = parser.parse_args()

    completecfg = load_cfg(args.completecfg)
    completecfg["clocks"] = Clocks(completecfg["clocks"])  # make clocks and resets correct type
    ...
    gen_top_docs(completecfg, c_helper, outdir)
