...
Each assembly source file is first assembled with acc_as.py. All resulting
objects are then linked with acc_ld.py. The resulting ELF file is converted
into an embeddable RV32 object file using objcopy.  In this object, all symbols
are prefixed with `_acc_app_<appname>_` (only global symbols are included).
...
environment variables:
  ...
  ACC_TOOLS         path to the ACC linker and assembler tools
  ...
  The RV32* environment variables are used by both this script and the ACC
  wrappers (acc_as.py and acc_ld.py) to find tools in a RV32 toolchain.

outputs:
  The build process produces multiple files inside the output directory.

  <src_file>.o            the compiled source files
  <app_name>.elf          the compiled and linked application targeting ACC
...
def run_tool(tool, out_file: Path, args) -> None:
    ...
    needed if we need to run multiple acc_build processes that generate the
...
def call_acc_as(src_file: Path, out_file: Path):
    run_tool(acc_as.main, out_file, [src_file])
...
def call_acc_ld(src_files: List[Path], out_file: Path,
    ...
    args += src_files
    run_tool(acc_ld.main, out_file, args)
...
def get_app_checksum(elf_path: str) -> int:
    '''Get the CRC32 checksum of an ACC app.
    ...
    the value is for IMEM or DMEM, `addr` is the word-offset within ACC's
...
def get_acc_syms(elf_path: str) -> List[Tuple[str, int]]:
    ...
    try:
        if not args.no_assembler:
            for src_file, obj_file in zip(src_files, obj_files):
                call_acc_as(src_file, obj_file)

        out_elf = out_dir / (app_name + '.elf')
        call_acc_ld(obj_files, out_elf, linker_script=args.linker_script)

        # out_elf is a fully-linked ACC binary, but we want to be able to use
        ...
        # to manipulate ACC data sections "in place") and add a .rodata.acc
        ...
        # against multiple ACC applications, we give the symbols an
        ...
        # to add (absolute) symbols that have the ACC addresses of the symbols
        ...
        # the objcopy.
        host_side_pfx = '_acc_local_app_{}_'.format(app_name)
        acc_side_pfx = '_acc_remote_app_{}_'.format(app_name)
        ...
        args += ['--add-symbol', f'{acc_side_pfx}_checksum={checksum:#x}']
