...
The exec\_envs are provided to the `pavona_binary` and `pavona_test` rules in order to allow them to perform their function.
...
The `pavona_binary` rule builds and links the binary for each provided exec\_env.
...
Because the `pavona_binary` rule emits several providers, any rule that accepts the outputs of `pavona_binary` must be prepared to deal with the providers it emits.
...
## The `pavona_test` rule

The `pavona_test` rule is actually a macro exported by `defs.bzl`.
There is a real `_pavona_test` rule in `cc.bzl`, but users are expected to use the macro.
