...
def _bitstream_splice_impl(ctx):
    ...
    if second_rom and second_rom.label.name != "none":
        ...
        mem = gen_vivado_mem_file(
            ...
            tool = tc.tools.gen_mem_image,
            swap_nibbles = ctx.attr.swap_nibbles,
            # The second ROM BRAMs in bitstreams for dual-ROM designs are
            # "crumb-endian". In other words, not only are the nibbles within a
            # byte swapped, so are the 2-bit "crumbs" within each nibble.
            #
            # For example, the byte 0b1100_1001 becomes 0b0110_0011.
            swap_crumbs = True,
        )
        src = vivado_updatemem(
            ctx = ctx,
