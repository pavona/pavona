...
However, flash memory accesses are not allowed, neither directly by the host or indirectly through flash protocol controller initiated transactions.
...
- Standard faults (found in [`STD_FAULT_STATUS`](registers.md#std_fault_status)).
- Custom faults (found in [`FAULT_STATUS`](registers.md#fault_status)).
...
See [flash escalation](#flash-escalation) for further differentiation between standard and custom faults.
...
Since controller operations are initiated through writes to the register file, the command check is identical to [host direct access to register files](#host-direct-access-to-flash-controller-register-files).
...
Local escalation is triggered by the standard faults of flash, seen in [`STD_FAULT_STATUS`](registers.md#std_fault_status).
...
- The host-facing TL-UL adapter errors back all host initiated operations.
...
*  If the data content is not scrambled, the PRINCE cipher and XOR steps are skipped and data provided directly back to the requester.
...
Scramble enabling is done differently depending on the type of partitions.
*  For data partitions, the scramble enabling is done on contiguous page boundaries.
   *  Software has the ability to configure these regions and whether scramble is enabled.
*  For information partitions, the scramble enabling is done on a per page basis.
...
The following diagram shows how the various ICV / ECC tags are applied and used through the life of a transaction.
![Flash ECC_LIFE](../doc/flash_integrity.svg).

Note that the ICV (integrity ECC) is calculated over the de-scrambled data and is only 4-bits, while the reliability ECC is calculated over both the scrambled data and the ICV.
...
This is why the data is calculated over the de-scrambled data as it can be stored alongside for continuous checks.
When de-scrambled data is returned to the host, the ICV is used to validate the data is correct.
...
ECC enabling is done differently depending on the type of partitions.
*  For data partitions, the ECC enabling is done on contiguous page boundaries.
   *  Software has the ability to configure these regions and whether ECC is enabled.
*  For information partitions,the ECC enabling is done on a per page basis.
...
*  When the flash read completes, its de-scrambling attributes are checked:
   * If de-scrambling is required, the read data begins the de-scrambling phase - at this time, a new flash read can be issued for the following transaction.
   * If de-scrambling is not required, the de-scrambling phase is skipped and the transaction is pushed to the response queue.
*  When the descrambling is complete, the de-scrambled text is pushed to the response queue.
...
When a read hits in the flash buffer, the integrity ECC is checked against the de-scrambled data and an error is returned to the initiating entity, whether it is the controller itself or a host.
