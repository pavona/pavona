...
static const top_egret_plic_target_t kHart = kTopEgretPlicTargetIbex0;
 ...
 * as the ISR.
 */
static volatile top_egret_plic_peripheral_t peripheral_expected;
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  switch (peripheral) {
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralAcc: {
      dif_acc_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralAdcCtrlAon: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralAlertHandler: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralAonTimerAon: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralCsrng: {
      dif_csrng_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralEdn0: {
      dif_edn_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralEdn1: {
      dif_edn_irq_t irq =
          (dif_edn_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralFlashCtrl: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralGpio: {
      dif_gpio_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralHmac: {
      dif_hmac_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralI2c1: {
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralI2c2: {
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralKeymgr: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralKmac: {
      dif_kmac_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralOtpCtrl: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralPattgen: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralPwrmgrAon: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralRvTimer: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralSensorCtrlAon: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralSpiDevice: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralSpiHost0: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralSpiHost1: {
      dif_spi_host_irq_t irq =
          (dif_spi_host_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralUart1: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralUart2: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralUart3: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
    case kTopEgretPlicPeripheralUsbdev: {
...
static void peripheral_irqs_trigger(void) {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralAcc;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralAdcCtrlAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralAlertHandler;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopEgretPlicPeripheralAonTimerAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralCsrng;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralEdn0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralEdn1;
  for (dif_edn_irq_t irq = kDifEdnIrqEdnCmdReqDone; irq <= kDifEdnIrqEdnFatalErr;
       ++irq) {
    edn_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralEntropySrc;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralFlashCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralGpio;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralHmac;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralI2c0;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralI2c1;
  status_default_mask = 0x0;
  for (dif_i2c_irq_t irq = kDifI2cIrqFmtThreshold; irq <= kDifI2cIrqHostTimeout;
       ++irq) {
    i2c_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralI2c2;
  status_default_mask = 0x0;
  for (dif_i2c_irq_t irq = kDifI2cIrqFmtThreshold; irq <= kDifI2cIrqHostTimeout;
       ++irq) {
    i2c_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralKeymgr;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralKmac;
  status_default_mask = 0x0;
  ...
  if (kBootStage != kBootStageOwner) {
    peripheral_expected = kTopEgretPlicPeripheralOtpCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralPattgen;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralPwrmgrAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralRvTimer;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralSensorCtrlAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralSpiDevice;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralSpiHost0;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralSpiHost1;
  status_default_mask = 0x0;
  for (dif_spi_host_irq_t irq = kDifSpiHostIrqError; irq <= kDifSpiHostIrqSpiEvent;
       ++irq) {
    spi_host_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralSysrstCtrlAon;
  status_default_mask = 0x0;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopEgretPlicPeripheralUart0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralUart1;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralUart2;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralUart3;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopEgretPlicPeripheralUsbdev;
  status_default_mask = 0x0;
...
bool test_main(void) {
  ...
  rv_plic_testutils_irq_range_enable(
      &plic, kHart, kTopEgretPlicIrqIdNone + 1, kTopEgretPlicIrqIdLast);
