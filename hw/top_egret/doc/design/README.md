# Egret Chip Specification

This document describes the Egret chip functionality in detail.
For an overview, refer to the [datasheet](../datasheet.md).

# Theory of Operations

The netlist `chip_egret_asic` contains the features listed in the datasheet and is intended for ASIC synthesis, whereas the netlist `chip_egret_cw310` provides an emulation environment for the `cw310` FPGA board.
...
`ast` supplies a number of clocks into `top_egret`.
...
`ast` also supplies a number of power-okay signals to `top_egret`, and these are used as asynchronous root resets.
- `vcaon_pok`: The always on domain of the system is ready.
- `vcmain_pok`: The main operating domain of the system is ready.

When one of these power-okay signals drop, the corresponding domain in `top_egret` is reset.
...
While the `ast` supplies clocks and resets to the `top_egret`, it also contains additional functions that interact with the design.
...
| AST Port         | `top_egret` Power Domain |
|------------------|-----------------------------|
...
Since `top_egret` contains flash, it is important to examine the memory's relationship with resets.
...
One feature available for Egret processor core is debug access.
...
Egret contains 1024kB of embedded-flash (e-flash) memory for code storage.
...
Egret, as an FPGA proof of concept, will model these parameters in its emulation of the memory in order to prepare for the replacement with the silicon flash macros that will come.
...
Egret follows the [Secure Boot](../../../../doc/security/specs/secure_boot/README.md) specification.
...
Egret contains a suite of "peripherals", or subservient execution units connected to the Ibex processor by means of a bus interconnect.
...
It is notable that there are many differences between an FPGA implementation of Egret and an ASIC version when it comes to pins and pads.
...
In order to be able to command I2C devices on systems where Egret will be included, I2C host functionality will be required.
...
The pins of the I2C host will be available to connect to any of the multiplexable IO (MIO) of the Egret device.
...
#### Topology
`top_egret` has a two-level hierarchy for its bus network.
...
For the purpose of `top_egret`, the first option has been chosen to benefit software development and testing.

## Entropy Distribution Network

`top_egret` has two [EDN](../../../ip/edn/README.md) instances, which are connected to one [CSRNG](../../../ip/csrng/README.md) instance (which in turn is connected to one [Entropy Source](../../../ip/entropy_src/README.md) instance).
The first EDN instance, `u_edn0`, has eight endpoints, which are connected to: Key Manager, OTP Controller, Analog Sensor Top, KMAC Accelerator, Alert Handler, AES Accelerator, the `URND` port of ACC, and the Ibex RISC-V Core.
The second EDN instance, `u_edn1`, has one endpoint that is connected to the `RND` port of ACC and seven endpoints that are tied off.
...
<!-- BEGIN CMDGEN util/design/gen-top-docs.py -t hw/top_egret/data/autogen/top_egret.gen.hjson -g pinout -->
...
At this time, the top-level netlist for Egret is a combination of hand-written SystemVerilog RTL with auto-generated sections for wiring of comportability interfaces.
There is a script for this auto-generation, centered around the top-level descriptor file `top_egret.hjson` found in the repository.
...
There is netlist automation to create the module `top_egret` as indicated in sections of this specification that follow.
...
This includes `chip_egret_cw310` for use in FPGA, and `chip_egret_asic` for use (eventually) in a silicon implementation.
...
In the FPGA platform, the logic for the JTAG and the SPI device are multiplexed within `chip_egret_cw310`.
