...
interface ast_supply_if (
  ...
  // index of key0 input pin in top_egret
  localparam int DioSysrstCtrlAonEcRstL = 10;
  // index of key input pin in top_egret
...
`ifndef GATE_LEVEL
    ...
    $asserton(1, top_egret.u_pwrmgr_aon.u_slow_fsm.IntRstReq_A);
`endif
  endtask
...
`ifndef GATE_LEVEL
    ...
    if (!value) begin
      ...
      $assertoff(1, top_egret.u_pwrmgr_aon.u_slow_fsm.IntRstReq_A);
    end
...
`undef GLITCH_VCMAIN_POK
  ...
  task static force_key0_i(bit value);
    ...
    force top_egret.mio_in_i[MioInSysrstCtrlAonKey0In] = value;
  endtask
