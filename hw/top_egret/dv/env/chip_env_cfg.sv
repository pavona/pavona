...
class chip_env_cfg #(type RAL_T = chip_ral_pkg::chip_reg_block) extends cip_base_env_cfg #(
  ...
  virtual function void initialize(bit [TL_AW-1:0] csr_base_addr = '1);
    ...
    debugger.num_triggers = 4;  // TODO: wire this from `top_egret_pkg`.
    ...
    foreach (sw_images[i]) begin
      if ("prebuilt" inside {sw_image_flags[i]}) begin
        ...
        if (i == SwTypeRom) begin
          ...
          // If test type is `pavona_test` and the flash image was generated
          // by the `pavona_test` Bazel macro itself, or it was generated
          // by the `pavona_flash_binary` or `pavona_binary` Bazel macros,
          // then we only need to append the device suffix.
          end else if ("ot_flash_binary" inside {sw_image_flags[i]} ||
              "new_rules" inside {sw_image_flags[i]}) begin
            sw_images[i] = $sformatf("%0s_%0s", sw_images[i], sw_build_device);
          // If test type is `pavona_functest` (the legacy OpenTitan Bazel
          // test macro), and said macro generated the flash image, then we
          // need to tweak the name, as the flash binary target has the `_prog`
          // suffix attached (to differentiate between the `sh_test` target
          // also generated by the same Bazel `pavona_functest` macro).
          end else begin
          ...
          if ("signed" inside {sw_image_flags[i]}) begin
            // Options match DEFAULT_SIGNING_KEYS in `rules/pavona/keyutils.bzl`.
