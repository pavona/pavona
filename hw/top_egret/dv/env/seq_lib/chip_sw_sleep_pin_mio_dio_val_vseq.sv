...
class chip_sw_sleep_pin_mio_dio_val_vseq extends chip_sw_base_vseq;
  ...
  pad_ret_t [top_egret_pkg::MioPadCount-1:0] mio_pad_ret;
  pad_ret_t [top_egret_pkg::DioCount-1:0]    dio_pad_ret;
  ...
  function void check_pad_retention_out();
    for (int i = 0; i < top_egret_pkg::MioPadCount; i++) begin
    ...
    for (int i = 0; i < top_egret_pkg::DioCount; i++) begin
      string msg = $sformatf("for DIO[%0d]", i);
      // Note that dios_if enumerates the DIOs in chip_egret_asic, which is different from the
      // DIOs enumerated in pinmux. The former is provided by top_egret_pkg::dio_pad_e and the
      // latter, by top_egret_pkg::dio_e. The chip_common_pkg::DioToDioPadMap maps the pinmux's
  ...
  task check_pads_retention_type();
    logic [top_egret_pkg::MioPadCount-1:0] mio_pads;
    logic [top_egret_pkg::DioCount-1:0]    dio_pads;
