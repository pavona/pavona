...
class chip_sw_sram_ctrl_scrambled_access_vseq extends chip_sw_base_vseq;
  ...
  localparam uint BACKDOOR_RET_OFFSET = (top_egret_pkg::TOP_EGRET_SRAM_CTRL_RET_AON_RAM_SIZE_BYTES / 8);
  localparam uint BACKDOOR_MAIN_OFFSET = (top_egret_pkg::TOP_EGRET_SRAM_CTRL_MAIN_RAM_SIZE_BYTES / 8);
  ...
  localparam string SRAM_CTRL_RET_HDL_PATH = "tb.dut.top_egret.u_sram_ctrl_ret_aon";
  localparam string SRAM_CTRL_MAIN_HDL_PATH = "tb.dut.top_egret.u_sram_ctrl_main";
  localparam string OTP_CTRL_KDI_HDL_PATH = "tb.dut.top_egret.u_otp_ctrl.u_otp_ctrl_kdi";
  ...
  virtual task ret_backdoor_write(int addr);
    int retval;
    bit scr_key_valid;
    int offset = addr - top_egret_pkg::TOP_EGRET_SRAM_CTRL_RET_AON_RAM_BASE_ADDR;
  ...
  virtual task main_backdoor_write(int addr);
    int retval;
    bit scr_key_valid;
    int offset = addr - top_egret_pkg::TOP_EGRET_SRAM_CTRL_MAIN_RAM_BASE_ADDR;
    // `backdoor` comes after `pattern` which is `BACKDOOR_DATA_WORDS` long.
    offset += BACKDOOR_DATA_WORDS * 4;
    forever begin
