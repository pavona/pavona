...
int main(int argc, char **argv) {
  ...
  std::string top_scope("TOP.chip_sim_tb.u_dut.top_egret");
  ...
  // in chip_egret_verilator.sv.  It must be at least 2 cycles of clk_aon.
  simctrl.SetInitialResetDelay(20000);
  simctrl.SetResetDuration(10);

  std::cout << "Simulation of OpenTitan Egret" << std::endl
            << "=================================" << std::endl
            << std::endl;
