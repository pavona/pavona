...
The Pavona project contains a number of these low-level reuseable components, which we refer to as '***Primitives***'.
...
Primitives in Pavona exist at a low level of RTL abstraction where it may be desirable under different circumstances to swap-out implementations to obtain better optimized hardware.
...
Within the Pavona repository, each technology library has its own directory within `hw/ip/` with the prefix `prim`, such as `hw/ip/prim_xilinx/` for synthesis targeting Xilinx FPGAs.
...
Pavona utilizes the *[Fusesoc][]* build system and package manager to manage RTL at a fileset level.
...
The virtual core is given a unique identifier representing its vendor, library, name, and version ([VLNV][]), and does not exist as a named `.core` file in the tree.
...
If a fileset depends on a virtual core, a resolution process must take place at build time to determine which of the possible implementations becomes the chosen concrete implementation.
...
If there are multiple implementations present, the FuseSoc solver expects to be unambiguously signalled via [*mappings*](https://fusesoc.readthedocs.io/en/stable/user/build_system/mappings.html), and will result in a 'conflicting-requirements' build time error in the absence of such explicit directives.

If multiple mappings are used within the dependency tree without explicitly specifying which one should take precedence, the following build time error will be generated:
```
...
Mappings are described in more detail below.

Many targets will choose to resolve their virtual cores automatically by adding a single specific implementation core into their dependencies.
...
While this works with normal cores, we restrict its usage to specifically map virtual cores to an implementation of that virtual core.
Passing `--mapping=<VLNV>` via the CLI will cause the FuseSoC solver to use the mappings within the core specified by that VLNV to resolve virtual cores to concrete implementations.
...
The Xilinx-specific technology-dependent primitives are first added to the dependency tree by listing their concrete implementations under the depend keyword.
...
One specific use of mappings is the lints for each block, such as the lint target for the UART in `hw/ip/uart/uart.core`.
...
Mappings may be present in top-level core files, e.g. in `hw/top_earlgrey/top_earlgrey.core` (depicted below), to specialize block-level flows for top specific implementations as described previously.
...
[Resolution of Concrete Implementations](#resolution-of-concrete-implementations) outlines the ways you can select a technology library.
...
The script `utils/design/check-netlist.py` (described [here](../../../util/design/README.md#netlist-checker-script)) can be used to report a summary of size_only cells in a netlist.
