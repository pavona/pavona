...
To build the testbench, execute from the Pavona top level:

```sh
fusesoc --cores-root=. --verbose run --setup --build lowrisc:dv_verilator:aes_tb
```

On Homebrew-based macOS systems, expose the OpenSSL headers and library while building:

```sh
CPLUS_INCLUDE_PATH="$(brew --prefix openssl@3)/include" \
LIBRARY_PATH="$(brew --prefix openssl@3)/lib" \
fusesoc --cores-root=. run --setup --build lowrisc:dv_verilator:aes_tb
```

To execute the obtained Verilator binary, run:
