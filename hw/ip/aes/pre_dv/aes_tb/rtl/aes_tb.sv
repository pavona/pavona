...
#(
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      gcm_phase_q <= gcm_phase_e'('0);
      aes_mode_q <= aes_mode_e'('0);
      ...
      gcm_phase_q <= gcm_phase_e'(bus_req.wdata[5:0]);
    end
  end
  ...
  // Pavona's TL-UL shim converts inputs from the valid-hold protocol into TL-UL requests and
  // implements the same internal name/version register window expected by this testbench.
