...
- `rtl/aes_wrap_tb.sv`: SystemVerilog testbench, instantiates and drives the different AES wrapper, compares outputs, signals test end and result (pass/fail) to C++ via output ports.
- `cpp/aes_wrap_tb.cc`: Contains main function and instantiation of SimCtrl, reads output ports of DUT and signals simulation termination to Verilator.
