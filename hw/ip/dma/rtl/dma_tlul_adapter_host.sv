...
//
// dma_tlul_adapter_host is the wide (64-bit address) host-port flavor of
// tlul_adapter_host. It converts basic req/grant/rvalid into a wide TL-UL interface. If
...
//
// When MAX_REQS > 1 dma_tlul_adapter_host does not do anything to order responses from the TL-UL
...
#(
  ...
  input                              req_i,
  output logic                       gnt_o,
  input  logic [DmaTlAw-1:0]         addr_i,
  input  logic                       we_i,
  ...
  input  logic [DmaRsvdWidth-1:0]    user_rsvd_i,
  ...
  assign tl_out = '{
    ...
    a_mask:    tl_be,
    a_source:  tl_source,
    a_address: {addr_i[DmaTlAw-1:WordSize], {WordSize{1'b0}}},
    a_data:    wdata_i,
    a_user:    '{default: '0, data_intg: wdata_intg_i, instr_type: instr_type_i, rsvd: user_rsvd_i},
    d_ready:   1'b1
  };

  dma_tlul_cmd_intg_gen #(.EnableDataIntgGen (EnableDataIntgGen)) u_cmd_intg_gen (
    .tl_i(tl_out),
    .tl_o(tl_o)
  );
