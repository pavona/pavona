...
When Flash mode is selected, the command parser accepts the first byte of the SPI MOSI line then activates several flash submodules such as Status, JEDEC, Read command, and Upload function.
The Status logic processes the three Read Status commands.
The software may configure three bytes of the Flash Status CSR then the Status submodule returns the CSR data into the SPI MISO line.
The software may configure the Read Status commands' opcodes.
...
For example, JEDEC ID `EFh` follows twelve bytes of `7Fh` Continuous Codes, requiring a total thirteen bytes for the manufacturer ID.
The software may configure how many Continuous Codes are needed and the actual manufacturer ID.
...
If the received address falls into the software programmable mailbox address space, the logic fetches data not from the read buffer but from the mailbox buffer in the DPSRAM.

The software may configure command information slots to upload the command into the FIFOs and the payload buffer in the DPSRAM.
It may additionally let hardware set the BUSY bit in the Status register when the hardware uploads the command.

In Passthrough mode, the logic filters the incoming transaction if the transaction is not permitted.
The software may configure the logic to change a portion of the address or first 4 bytes of the payload.

## SPI Flash and Passthrough Modes

### Command Information List

The software may configure the map from the received opcode to the command process module by programming *cmd_info* list.
The current SPI device provides 24 command information entries.
Each entry represents a command.
Details of the fields are explained in the [`CMD_INFO_0`](registers.md#cmd_info) register.
...
In Passthrough mode, if Read Status and Read JEDEC ID commands are intercepted by the internal hardware, the other fields in the command information entries are ignored also.
...
The *upload* and *busy* fields are used in the SPI Flash/ Passthrough modes.
...
The command parser (*cmdparse*) processes the first byte of the SPI and activates the processing submodules depending on the received opcode and the *cmd_info* list described in the previous section.
...
Other than the first 11 slots and last two slots (which are not visible to software), the *cmdparse* checks the *upload* field and activates the upload module if the field is set.

The software can configure whether a submodule should process the command while in the passthrough mode by setting the [`INTERCEPT_EN`](registers.md#intercept_en) CSR.

### Status Control

If the received command is one of the three read status commands, the STATUS control module takes over the SPI interface after the opcode.
The 3 bytes status register is not reset by `CSb`.
Except for the BUSY and the WEL bit, other bits are controlled by software.

The BUSY bit is set by hardware when it receives any commands that are uploaded to the FIFOs and their `busy` fields are 1 in the command information entry.
Software may clear the BUSY bit when it completes the received commands (e.g Erase / Program).

If BUSY is set, the SPI device IP blocks the passthrough interface in Passthrough mode.
The blocking of the interface occurs in SPI transaction idle state (`CSb == 1`).
When software clears the BUSY bit, it is applied to the STATUS register in the SPI clock domain when the SPI clock toggles.
This means the update happens when the next SPI transaction is received.
The BUSY bit in the CSR is the synchronized value of the STATUS BUSY bit in the SPI clock domain.
Due to the CDC latency, the software may see the updated value (BUSY clear) with long delay.

The WEL bit can get updates from both hardware and software.
Hardware updates WEL bit when it receives WREN (`06h`) or WRDI (`04h`) commands.
The opcode can be configured via [`CMD_INFO_WREN`](registers.md#cmd_info_wren) and [`CMD_INFO_WRDI`](registers.md#cmd_info_wrdi).
Meanwhile, the software may clear the WEL bit when it completes the received commands.
Note that software may only clear the WEL bit.

The software update of the STATUS register via [`FLASH_STATUS`](registers.md#flash_status) is not instantaneous.
The IP stores the software request into the asynchronous FIFO then the request is processed in the SPI clock domain.
...
Software sees the committed registers when reading the [`FLASH_STATUS`](registers.md#flash_status) CSR.
...
A Read Status command can thus repeatedly poll the BUSY bit and see updates in the same transaction.
...
Software must configure the remaining command information entries to upload the Write Status commands to the FIFOs.

### JEDEC ID Control

JEDEC module returns JEDEC Device ID and Manufacturer ID following the Continuation Code (CC).
Software may configure the [`JEDEC_CC`](registers.md#jedec_cc) CSR for hardware to return proper CC.
The *cc* field in [`JEDEC_CC`](registers.md#jedec_cc) defines the return value, which is `0x7F` by default.
*num_cc* defines how many times the hardware has to send CC byte before sending the JEDEC ID.

The actual JEDEC ID consists of one byte manufacturer ID and two bytes device ID.
The hardware sends the manufacturer ID first, then `[7:0]` of the device ID then `[15:8]` byte.

### Serial Flash Discoverable Parameters (SFDP) Control

The hardware parses SFDP command then fetch the data from SFDP space in the DPSRAM.
It also provides 256B SFDP of space.
The hardware uses the lower 8-bit of the received 24-bit address to access the DPSRAM; the upper 16 bits are ignored (aliased).
The software should prepare proper SFDP contents before the host system issues SFDP commands.

Hardware fetches from the DPSRAM in 4B and returns the data to the SPI line.
Hardware repeats the operation until `CSb` is de-asserted.
...
If hardware receives the read access to the other half of the space first time, then the hardware reports to the software to refill the current 1kB region with new content.

The double buffering scheme aids the software to prepare the next chunk of data.
The software copies a portion of data (1kB) from the internal flash memory into the SPI device DPSRAM.
...
The software may configure the dummy cycle field for each individual read commands.
The default dummy cycle for those commands are 7 (0-based).
The value is the number of cycles.
For example, if software programs the dummy cycle for Fast Read Quad to `3h`, the module waits 4 cycles then returns data.
...
The IP only uses the lower 11 bits of the received read command address (`addr[10:0]`) to issue the read requests to the DPSRAM.
...
The software is responsible for updating the read buffer contents.
The hardware notifies the software to update the buffer contents when needed.
The hardware also provides a software configurable read watermark CSR and read-only [`LAST_READ_ADDR`](registers.md#last_read_addr) CSR.
...
The hardware assumes the software maintains the read buffer as a double buffer scheme.
When the host system accesses one buffer (1kB), the software prepares another 1kB by copying data from the internal non-volatile memory.
If the received read address crosses the software configured watermark address, the hardware informs the software.
Software may configure the watermark CSR low enough so that the software has enough time to copy over the data.

If a new read command crosses the current buffer boundary, the software flips the internal buffer index bit and clears the cross event for the hardware to detect the address cross event again.

### 4B Address Management (EN4B/ EX4B)

The software may configure the HW to receive EN4B and EX4B commands and change the read command address size between 3 bytes and 4 bytes.
...
The hardware logic creates the default **CMD_INFO** structures for the two commands.
The command parser module uses the generated structures to process and trigger the 4B management module.

When the hardware receives one of the commands, the hardware changes the broadcast signal *cfg_addr_4b_en*.
Also the hardware updates [`ADDR_MODE.addr_4b_en`](registers.md#addr_mode) after passing through CDC.
...
_Note: The hardware changes the broadcasting signal and the CSR even though the SPI host system sends more than 8 beats of the SPI `S[0]`.
...
Software may also configure the initial address mode by writing to [`ADDR_MODE.addr_4b_en`](registers.md#addr_mode).
The software must not write to this CSR unless it knows the upstream host is inactive and expects initial values for the next transaction.
...
A software request will set the [`ADDR_MODE.pending`](registers.md#addr_mode) bit, which will remain set until hardware consumes the update.
The software will see the intended mode in [`ADDR_MODE.addr_4b_en`](registers.md#addr_mode) until hardware completes the request.
However, software may overwrite the requested value, so long as the SPI host is held inactive.
...
If the received command meets following conditions, the hardware stores the command into the command/address FIFOs and the payload buffer:
...
The software may clear the *BUSY* bit after processing the command.

In addition, the hardware stores some metadata alongside the opcode in the [`CMD FIFO`](registers.md#upload_cmdfifo).
...
With these, the software can determine what size the address should be and whether the uploaded command should be rejected.

The upload module provides [`UPLOAD_STATUS`](registers.md#upload_status) and [`UPLOAD_STATUS2`](registers.md#upload_status2) CSRs for the software to parse the command, address, and payload.
If a received command has payload, the software may read the payload from the Payload buffer starting from `payload_start_idx` address.
...
In this scenario, the software should read from `payload_start_idx` to the end of the payload buffer then do a second read from the beginning of the buffer to the remained bytes.
...
The software does not know whether a SPI transaction is filtered or not.
If the software wants to check, it needs to set the _upload_ field with the opcode in the command information list.
Then, the hardware uploads the command into the command/address FIFOs and the payload buffer.

#### Address Manipulation

The software may configure the passthrough logic to swap certain address bits to desired values by configuring [`ADDR_SWAP_MASK`](registers.md#addr_swap_mask) and [`ADDR_SWAP_DATA`](registers.md#addr_swap_data) CSRs.
...
The software may configure the [`PAYLOAD_SWAP_MASK`](registers.md#payload_swap_mask) and [`PAYLOAD_SWAP_DATA`](registers.md#payload_swap_data) CSRs to change the specific bit of the first 4 byte of the write payload.
...
Software may use additional command information slots for the passthrough (index 11 to 23).
The software must configure *payload_dir* to **PayloadIn** and *payload_en* to `4'b 0001` in order for the payload translation feature to work correctly.
...
The information of the pad enable and direction is given by software.
Software configures the address size, payload lanes, dummy size in **CMD_INFO** slots.
...
Software is recommended to set the filter bit for Passthrough to not deliver the unmatched command to the downstream flash device.
...
Software may selectively enable/disable commands.
For example, hardware returns only Read Status data internally if [`INTERCEPT_EN`](registers.md#intercept_en) is `{status: 1'b 1, default: 1'b 0}`.
...
The TPM submodule parses the incoming SPI MOSI line and stacks the stream up to the software accessible registers, such as *TPM_CMD_ADDR* and the write FIFO buffer.
The software must decode the command and the address.
Then the software reads the data from the write FIFO or pushes data into the read FIFO depending on the command.
...
The module sends bytes from the return-by-HW registers to the parallel-to-serial logic right after the address phase when the received address falls into the hardware managed registers.
...
As the logic decides if the hardware returns data or waits for the software response at the address bit phase, the logic always sends `WAIT(0x00)` at the last byte of the incoming address phase.
...
The return-by-HW register values come from the software read-writable CSRs.
The module latches the CSRs from the `SYS_CLK` domain into the `SPI SCK` domain when `CSb` is asserted.
The software is allowed to modify the return-by-HW registers when `CSb` is not active.
...
The hardware uses `TPM_ACCESS_x.activeLocaltiy` and the address bits 15 through 12 to determine what value the logic should return.
...
Other registers in the table should be processed by software.
...
The hardware checks the parity error when the software reads data from the SRAM.
The error is reported to the software via the TL-UL D channel error signal.
Software is recommended to discard the current context if any transaction is ongoing; then to reset the IP.
