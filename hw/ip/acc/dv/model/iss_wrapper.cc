...
struct TmpDir {
 ...
 private:
  ...
  static std::string make_tmp_dir() {
    ...
    tmp_template += "/acc_XXXXXX";

    if (!mkdtemp(&tmp_template.at(0))) {
      std::ostringstream oss;
      oss << ("Cannot create temporary directory for ACC simulation "
              "with template ")
  ...
  // Return true if the ACC_MODEL_KEEP_TMP environment variable is set to 1.
  static bool should_keep_tmp() {
    const char *keep_str = getenv("ACC_MODEL_KEEP_TMP");
    if (!keep_str)
      return false;
  ...
  for (;;) {
    ...
    if (last_slash == 0) {
      ...
      oss << "Cannot find a git top-level directory containing "
          << self_path.get()
          << (". To run the ACC model outside of the repo, "
...
static void read_ext_reg(const std::string &reg_name,
  ...
  //
  //   ! acc.$REG_NAME: 0x00000000
  std::regex re("! acc\\." + reg_name + ": 0x([0-9a-f]{8})");
  std::smatch match;
...
ISSWrapper::ISSWrapper() : tmpdir(new TmpDir()) {
  std::string model_path(find_acc_model());
...
int ISSWrapper::step(bool gen_trace) {
  ...
  if (gen_trace && lines.size()) {
    if (!AccTraceChecker::get().OnIssTrace(lines)) {
      return -1;
    }
  }
