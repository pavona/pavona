...
class acc_env_cov extends cip_base_env_cov #(.CFG_T(acc_env_cfg));
  `uvm_component_utils(acc_env_cov)
...
`define DEF_CSR_CP(NAME, EXPR)                          \
  NAME: coverpoint (remap_csr(EXPR)) {                  \
    ...
    bins urnd          = {16};                          \
    bins invalid       = {-1};                          \
    ignore_bins no_pqc = {12, 13, 14} iff (!AccPQCEn); \
...
`define DEF_WSR_CP(NAME, EXPR)                                   \
  ...
  covergroup ext_csr_cmd_cg
    with function sample(acc_pkg::cmd_e     value,
    ...
    cmd_cp: coverpoint {value} {
      bins CmdExecute     = {acc_pkg::CmdExecute};
      bins CmdSecWipeDmem = {acc_pkg::CmdSecWipeDmem};
      bins CmdSecWipeImem = {acc_pkg::CmdSecWipeImem};
      bins BogusCmd       = {[0:$]} with (!(item inside {acc_pkg::CmdExecute,
                                                         acc_pkg::CmdSecWipeDmem,
                                                         acc_pkg::CmdSecWipeImem}));
    }
  ...
  covergroup ext_csr_status_cg
    with function sample(acc_pkg::status_e  value,
                         access_e            access_type);
  ...
  covergroup ext_csr_err_bits_cg
    with function sample(acc_pkg::err_bits_t value,
...
`undef DEF_FAC_CP
  ...
  covergroup scratchpad_writes_cg with function sample(uvm_reg_addr_t addr);
    ...
    addr_cp: coverpoint addr {
      bins low  = {ACC_DMEM_OFFSET + ACC_DMEM_SIZE};
      bins high = {ACC_DMEM_OFFSET + DmemSizeByte - 4};
    }
  endgroup
  ...
  covergroup bad_internal_state_cg
    with function sample(logic urnd_all_zero,
                         ...
                         acc_pkg::predec_err_t predec_err,
                         acc_pkg::missed_gnt_t missed_gnt,
                         acc_pkg::controller_bad_int_t controller_bad_int,
                         acc_pkg::start_stop_bad_int_t start_stop_bad_int,
  ...
  covergroup internal_intg_err_cg with function sample(acc_pkg::internal_intg_err_t intg_err);
  ...
  covergroup enc_bnmulv_cg
    ...
    // hw/ip/acc/data/bignum-insns.yml
  ...
  covergroup enc_bnmulvl_cg
    ...
    // be any .8 variant from hw/ip/acc/data/bignum-insns.yml
  ...
  function void on_ext_csr_access(uvm_reg csr, acc_env_pkg::access_e access_type,
    ...
    case (csr_name)
      "cmd": begin
        ext_csr_cmd_cg.sample(acc_pkg::cmd_e'(data), access_type, state);
      end
      "ctrl": begin
      ...
      "err_bits": begin
        last_err_bits = data;
        ext_csr_err_bits_cg.sample(acc_pkg::err_bits_t'(data),
  ...
  //
  function void on_insn(acc_model_item iss_item, acc_trace_item rtl_item);
    string encoding;
