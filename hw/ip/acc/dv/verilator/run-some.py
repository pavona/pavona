...
which will generate 10 ACC binaries, each with up to 1500 instructions in
their respective traces. It will also build a Verilated model of ACC (using
acc_top_sim) and run the model on each binary.
...
def write_ninja(handle: TextIO,
    ...
    handle.write('rule fusesoc\n'
                 ...
                 '>fusesoc.log 2>&1\n\n')
    handle.write('tb = build/lowrisc_ip_acc_top_sim_0.1/'
                 'sim-verilator/Vacc_top_sim\n\n')
