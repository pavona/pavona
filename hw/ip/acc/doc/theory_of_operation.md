...
![ACC architecture block diagram](./acc_blockarch.svg)
...
ACC employs an instruction prefetch stage to enable pre-decoding of instructions to enable the [blanking SCA hardening measure](../README.md#blanking).
...
1. The currently running operation is terminated, similar to the way an {{#acc-insn-ref ECALL}} instruction [is executed](#software-execution):
...
2. If ACC [is not idle](#operational-states), then the currently running operation is terminated, similarly to how an operation ends after an {{#acc-insn-ref ECALL}} instruction [is executed](#software-execution):
   - No more instructions are fetched or executed.
   - A [secure wipe of internal state](#internal-state-secure-wipe) is performed.
   - The [`ERR_BITS`](registers.md#err_bits) register is set to a non-zero value that describes the error.
   - The current operation is marked as complete by setting [`INTR_STATE.done`](registers.md#intr_state).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>fatal</td>
      <td>The internal state of ACC has become corrupt.</td>
    </tr>
    <tr>
...
ACC receives and reacts to escalation signals from the [life cycle controller](../../lc_ctrl/doc/theory_of_operation.md#security-escalation).
...
For example, see egret's [OTP block](../../../top_egret/ip_autogen/otp_ctrl/doc/theory_of_operation.md#scrambling-key-derivation).
...
The ephemeral memory scrambling key and the nonce are provided by the OTP block (for example, see egret's [scrambling specification](../../../top_egret/ip_autogen/otp_ctrl/doc/theory_of_operation.md#scrambling-key-derivation)).
