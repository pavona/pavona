...
A `.macro`/`.irp`/`.irpc` parameter may be used as a `bn.*` register
operand, e.g. `bn.xor \a, \a, \b` inside `.macro foo a, b`: `acc_as.py`
defers the encoding to the real assembler, so `.if`, `.exitm` and recursive
macros all work as usual. Not supported: `.altmacro`, and a parameterized
mnemonic, immediate, or enum/option/csr/wsr operand.
...
The software running on ACC signals completion by executing the {{#acc-insn-ref ECALL}} instruction.

Once ACC has executed the {{#acc-insn-ref ECALL}} instruction, the following things happen:
...
ACC provides two hardware loop instructions: {{#acc-insn-ref LOOP}} and {{#acc-insn-ref LOOPI}}.
...
This section outlines two techniques to perform larger multiplies by composing multiple {{#acc-insn-ref BN.MULQACC}} instructions.
