 ...
 */
// Below countermeasure (no data dependent control flow in ACC ISA) is inherent to the design and
// has no directly associated RTL
// SEC_CM: CTRL_FLOW.SCA
module acc_core
  import acc_pkg::*;
#(
  // Register file implementation selection, see acc_pkg.sv.
  ...
  `ASSERT_STATIC_LINT_ERROR(AccSecMuteUrndNonDefault, SecMuteUrnd == 0)
  ...
  // Start stop control start ACC execution when requested and deals with any pre start or post
  // stop actions.
  ...
  acc_instruction_fetch #(
    .ImemSizeByte(ImemSizeByte),
    .AccPQCEn(AccPQCEn)
  ) u_acc_instruction_fetch (
    .clk_i,
    .rst_ni,
  ...
  `ASSERT(InsnDataStableInStall, u_acc_controller.state_q == AccStateStall |->
  ...
  // modules in acc_core. Note that each error signal except escalate_en_i that appears here also
  ...
  `ASSERT(AccStartStopGlobalEscCntrMeasure_A, err_bits_q && mubi4_test_true_loose(escalate_en_i)
          && mubi4_test_true_loose(start_stop_escalate_en)|=> ##[1:4000]
          u_acc_start_stop_control.state_q == acc_pkg::AccStartStopStateLocked)

  `ASSERT(AccStartStopLocalEscCntrMeasure_A, err_bits_q && mubi4_test_false_strict(escalate_en_i)
          && mubi4_test_true_loose(start_stop_escalate_en) |=>  ##[1:4000]
          u_acc_start_stop_control.state_q == acc_pkg::AccStartStopStateLocked)
  ...
  `ASSERT(AccControllerGlobalEscCntrMeasure_A, err_bits_q && mubi4_test_true_loose(escalate_en_i)
          && mubi4_test_true_loose(controller_fatal_escalate_en)|=> ##[1:100]
          u_acc_controller.state_q == acc_pkg::AccStateLocked)

  `ASSERT(AccControllerLocalEscCntrMeasure_A, err_bits_q && mubi4_test_false_strict(escalate_en_i)
          && mubi4_test_true_loose(controller_fatal_escalate_en) |=>  ##[1:100]
          u_acc_controller.state_q == acc_pkg::AccStateLocked)
