...
#(
        ...
        InsnOpcodeBignumArith: begin
          unique case (imem_rdata_i[14:12])
            ...
            3'b101: begin
              ...
              // rdata[25] can not be 1 and AccPQCEn be 0
              if (~imem_rdata_i[25] | AccPQCEn) begin
                rf_ren_a_bignum                = 1'b1;
                rf_ren_b_bignum                = 1'b1;
                rf_we_bignum                   = 1'b1;
                alu_bignum_shift_amt           = shift_amt_a_type_bignum;
          ...
          unique case (imem_rdata_i[14:12])
            ...
            3'b110: begin
              if (imem_rdata_i[31]) begin // BN.MOVR
                ...
                // register access so aren't set here. acc_controller sets the appropriate read and
    ...
    if (csr_addr_sel) begin
      ...
      unique case (wsr_addr)
        ...
        WsrKmacDigest0: begin
          if (AccPQCEn) begin
            ispr_addr = IsprKmacDigest0;
          end
        end
        WsrKmacDigest1: begin
          if (AccPQCEn) begin
            ispr_addr = IsprKmacDigest1;
          end
        end
        WsrAccH: begin
          if (AccPQCEn) begin
  ...
  assign alu_predec_bignum_o.vector_type = AccPQCEn ? alu_bignum_vector_type_pqc
                                                     : alu_vector_type_t'(3'b000);
  assign alu_predec_bignum_o.vector_sel  = AccPQCEn ? alu_bignum_vector_sel_pqc : '0;
  assign alu_predec_bignum_o.trn_type    = AccPQCEn ? alu_bignum_trn_type_pqc
