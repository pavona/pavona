...
#(
  ...
  parameter otp_ctrl_pkg::acc_key_t   RndCnstAccKey   = RndCnstAccKeyDefault,
  parameter otp_ctrl_pkg::acc_nonce_t RndCnstAccNonce = RndCnstAccNonceDefault
) (
  input clk_i,
  input rst_ni,
  ...
  input                                   clk_otp_i,
  input                                   rst_otp_ni,
  output otp_ctrl_pkg::acc_otp_key_req_t acc_otp_key_o,
  input  otp_ctrl_pkg::acc_otp_key_rsp_t acc_otp_key_i,
  input  keymgr_pkg::acc_key_req_t       keymgr_key_i,
  ...
  if (Stub) begin : gen_stub_acc
    assign rst_n = 1'b0;
  end else begin : gen_real_acc
    assign rst_n = rst_ni;
  end

  // The ACC_*_SIZE parameters are auto-generated by regtool and come from the bus window sizes;
  // they are given in bytes and must be powers of two.
  //
  // DMEM is actually a bit bigger than ACC_DMEM_SIZE: there are an extra DmemScratchSizeByte bytes
  // that aren't accessible over the bus.
  localparam int ImemSizeByte = int'(acc_reg_pkg::ACC_IMEM_SIZE);
  localparam int DmemSizeByte = int'(acc_reg_pkg::ACC_DMEM_SIZE + DmemScratchSizeByte);
  ...
  // Bus device windows, as specified in acc.hjson
  typedef enum logic {
    TlWinImem = 1'b0,
    TlWinDmem = 1'b1
  } tl_win_e;
  ...
  generate
    if (AccPQCEn) begin : gen_active_app_intf
  ...
  // The clock can be gated and some registers can be updated as long as ACC isn't currently
  // running. Other registers can only be updated when ACC is in the Idle state (which also implies
  // we are not locked).
  logic is_not_running_d, is_not_running_q;
  logic acc_dmem_scramble_key_req_busy, acc_imem_scramble_key_req_busy;

  assign is_not_running_d = ~|{busy_execute_d,
                               acc_dmem_scramble_key_req_busy,
                               acc_imem_scramble_key_req_busy,
                               busy_secure_wipe};

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if(!rst_ni) begin
      // ACC starts busy, performing the initial secure wipe.
  ...
  // Internally, ACC uses MUBI types.
  ...
  otp_ctrl_pkg::acc_key_t acc_imem_scramble_key;
  acc_imem_nonce_t        acc_imem_scramble_nonce;
  logic                    acc_imem_scramble_valid;
  logic                    unused_acc_imem_scramble_key_seed_valid;

  otp_ctrl_pkg::acc_key_t acc_dmem_scramble_key;
  acc_dmem_nonce_t        acc_dmem_scramble_nonce;
  logic                    acc_dmem_scramble_valid;
  logic                    unused_acc_dmem_scramble_key_seed_valid;


  logic acc_scramble_state_error;

  // SEC_CM: SCRAMBLE.KEY.SIDELOAD
  acc_scramble_ctrl #(
    .RndCnstAccKey  (RndCnstAccKey),
    .RndCnstAccNonce(RndCnstAccNonce)
  ) u_acc_scramble_ctrl (
    .clk_i,
    .rst_ni,
    ...
    .acc_dmem_scramble_key_o           (acc_dmem_scramble_key),
    .acc_dmem_scramble_nonce_o         (acc_dmem_scramble_nonce),
    .acc_dmem_scramble_valid_o         (acc_dmem_scramble_valid),
    .acc_dmem_scramble_key_seed_valid_o(unused_acc_dmem_scramble_key_seed_valid),

    .acc_imem_scramble_key_o           (acc_imem_scramble_key),
    .acc_imem_scramble_nonce_o         (acc_imem_scramble_nonce),
    .acc_imem_scramble_valid_o         (acc_imem_scramble_valid),
    .acc_imem_scramble_key_seed_valid_o(unused_acc_imem_scramble_key_seed_valid),

    .acc_dmem_scramble_sec_wipe_i    (dmem_sec_wipe),
    .acc_dmem_scramble_sec_wipe_key_i(dmem_sec_wipe_urnd_key),
    .acc_imem_scramble_sec_wipe_i    (imem_sec_wipe),
    .acc_imem_scramble_sec_wipe_key_i(imem_sec_wipe_urnd_key),

    .acc_dmem_scramble_key_req_busy_o(acc_dmem_scramble_key_req_busy),
    .acc_imem_scramble_key_req_busy_o(acc_imem_scramble_key_req_busy),

    .state_error_o(acc_scramble_state_error)
  );
  ...
  prim_ram_1p_scr #(
    ...
    .key_valid_i(acc_imem_scramble_valid),
    .key_i      (acc_imem_scramble_key),
    .nonce_i    (acc_imem_scramble_nonce),
  ...
  logic imem_gnt_bus;
  // Always grant to bus accesses, when ACC is running a dummy response is returned
  ...
  // instruction from IMEM through the bus unintentionally. Also blank when ACC is returning
  // a dummy response (responding to an illegal bus access) and when ACC is locked.
  ...
  // When ACC is locked all imem bus reads should return 0. The blanker produces the 0s, this adds
  // the appropriate ECC. When ACC is not locked the output of the blanker is passed straight
  ...
  localparam int DmemBusSizeWords = int'(acc_reg_pkg::ACC_DMEM_SIZE) / (WLEN / 8);
  ...
  prim_ram_1p_scr #(
    ...
    .key_valid_i(acc_dmem_scramble_valid),
    .key_i      (acc_dmem_scramble_key),
    .nonce_i    (acc_dmem_scramble_nonce),
  ...
  for (genvar i_word = 0; i_word < BaseWordsPerWLEN; ++i_word) begin : g_dmem_intg_check
    ...
    // that ACC isn't using will cause false errors. dmem_rerror is only reported for reads from
    // ACC. For Ibex reads integrity checking on TL responses will serve the same purpose.
  ...
  // unintentionally. Also blank when ACC is returning a dummy response (responding to an illegal
  // bus access) and when ACC is locked.
  ...
  // When ACC is locked all dmem bus reads should return 0. The blanker produces the 0s, this adds
  // the appropriate ECC. When ACC is not locked the output of the blanker is passed straight
  // through. Data bits are always left un-modified. A registered version of `locking` is used for
  // timing reasons. When a read comes in when `locking` has just been asserted, `locking_q` will be
  // timing reasons. When a read comes in when `locking` has just been asserted, `locking_q` will be
  ...
  // CMD register
  always_comb begin
    // start is flopped to avoid long timing paths from the TL fabric into ACC internals.
    start_d       = 1'b0;
    dmem_sec_wipe = 1'b0;
    imem_sec_wipe = 1'b0;
    ...
    if (status_q == StatusIdle) begin
      ...
      // ACC can command a secure wipe of IMEM and DMEM. This occurs when ACC encounters a fatal
      // error.
  ...
  assign status_d = locking                         ? StatusLocked          :
                    ...
                    acc_dmem_scramble_key_req_busy ? StatusBusySecWipeDmem :
                    acc_imem_scramble_key_req_busy ? StatusBusySecWipeImem :
                                                      StatusIdle;
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
  ...
  // acc_dmem_scramble_key_req_busy, acc_imem_scramble_key_req_busy} are possible.
  //
  ...
  `ASSERT(NoStartWhenLocked_A,
          locking |=> !($rose(busy_execute_d) ||
                        $rose(acc_dmem_scramble_key_req_busy) ||
                        $rose(acc_imem_scramble_key_req_busy)))
  ...
  // ERR_BITS register
  // The error bits for an ACC operation get stored on the cycle that done is
...
`undef DEF_FAC_BIT
  ...
  assign non_core_err_bits = '{
    ...
    bad_internal_state:   acc_scramble_state_error | missed_gnt_error_q,
  ...
  // 1. urnd_reseed_err disables the assertion because secure wipe finishes with failure and ACC
  ...
  for (genvar i = 2; i < NGpr; ++i) begin : gen_sec_wipe_gpr_asserts
    ...
    `ASSERT(InitSecWipeNonZeroBaseRegs_A,
      ...
      u_acc_core.u_acc_rf_base.gen_rf_base_ff.u_acc_rf_base_inner.g_rf_flops[i].rf_reg_q !=
        EccZeroWord,
      clk_i,
      !rst_ni || u_acc_core.urnd_reseed_err || u_acc_core.u_acc_start_stop_control.mubi_err_d ||
      u_acc_core.u_acc_start_stop_control.state_error_d)
  ...
  if (1) begin : g_secure_wipe_assertions
    ...
    for (genvar i = 0; i < NWdr; ++i) begin : gen_sec_wipe_wdr_asserts
      ...
      `ASSERT(InitSecWipeNonZeroWideRegs_A,
              $fell(busy_secure_wipe) |->
                u_acc_core.u_acc_rf_bignum.gen_rf_bignum_ff.u_acc_rf_bignum_inner.rf[i] !=
                  EccWideZeroWord,
              clk_i,
              !rst_ni || u_acc_core.urnd_reseed_err ||
                u_acc_core.u_acc_start_stop_control.mubi_err_d ||
                u_acc_core.u_acc_start_stop_control.state_error_d)
      ...
      `ASSERT(SecWipeChangedWideRegs_A,
              $rose(busy_secure_wipe) |-> ((##[0:$]
                ...
                $changed(
                  u_acc_core.u_acc_rf_bignum.gen_rf_bignum_ff.u_acc_rf_bignum_inner.rf[i]))
                within ($rose(busy_secure_wipe) ##[0:$] $fell(busy_secure_wipe))),
            clk_i, !rst_ni || u_acc_core.urnd_reseed_err ||
              u_acc_core.u_acc_start_stop_control.mubi_err_d)
    end
  end
  ...
  `ASSERT(SecWipeInvalidCallStack_A,
          $fell(busy_secure_wipe) |-> (!u_acc_core.u_acc_rf_base.u_call_stack.top_valid_o),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d ||
            u_acc_core.u_acc_start_stop_control.state_error_d ||
  ...
  `ASSERT(SecWipeInvalidLoopStack_A,
          $fell(busy_secure_wipe) |->
            (!u_acc_core.u_acc_controller.u_acc_loop_controller.loop_info_stack.top_valid_o),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d ||
            u_acc_core.u_acc_start_stop_control.state_error_d ||
  ...
  `ASSERT(SecWipeNonZeroMod_A,
          $fell(busy_secure_wipe) |-> u_acc_core.u_acc_alu_bignum.mod_intg_q != EccWideZeroWord,
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d ||
            u_acc_core.u_acc_start_stop_control.state_error_d ||
  ...
  `ASSERT(SecWipeNonZeroACC_A,
          $fell(busy_secure_wipe) |->
            u_acc_core.u_acc_alu_bignum.ispr_acc_intg_i != EccWideZeroWord,
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d ||
            u_acc_core.u_acc_start_stop_control.state_error_d)
  ...
  `ASSERT(SecWipeNonZeroFlags_A,
          $fell(busy_secure_wipe) |-> (!u_acc_core.u_acc_alu_bignum.flags_flattened),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d ||
            u_acc_core.u_acc_start_stop_control.state_error_d ||
  ...
  `ASSERT(ImemSecWipeRequiresUrndKey_A,
          $rose(imem_sec_wipe) |=> (acc_imem_scramble_key == $past(imem_sec_wipe_urnd_key)),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d)
  `ASSERT(DmemSecWipeRequiresUrndKey_A,
          $rose(dmem_sec_wipe) |=> (acc_dmem_scramble_key == $past(dmem_sec_wipe_urnd_key)),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d)
  ...
  `ASSERT(ImemSecWipeRequiresOtpKey_A,
          $rose(imem_sec_wipe) ##1 (acc_imem_scramble_key == $past(imem_sec_wipe_urnd_key)) |=>
            ##[0:$] acc_imem_scramble_valid && $changed(acc_imem_scramble_key),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d)
  `ASSERT(DmemSecWipeRequiresOtpKey_A,
          $rose(dmem_sec_wipe) ##1 (acc_dmem_scramble_key == $past(dmem_sec_wipe_urnd_key)) |=>
            ##[0:$] acc_dmem_scramble_valid && $changed(acc_dmem_scramble_key),
          clk_i,
          !rst_ni || u_acc_core.urnd_reseed_err ||
            u_acc_core.u_acc_start_stop_control.mubi_err_d)
  ...
  `ASSERT_KNOWN(AccOtpKeyO_A, acc_otp_key_o, clk_otp_i, !rst_otp_ni)
