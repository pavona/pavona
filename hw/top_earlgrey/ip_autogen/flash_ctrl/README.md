...
* Fully open-source flash controller
* Open-source flash wrapper module, with closed-source CSRs
* Open-source simulation flash module
...
* Flash protocol controller (`flash_ctrl.sv`)
* Flash physical controller (`flash_phy.sv`)
...
The flash protocol controller interfaces with software and other hardware components in the system (such as life cycle controller, key manager and OTP controller).
...
*  Parameterized support for burst program / read, up to 64B.
   *  Longer programs / reads are supported, however the protocol controller will directly apply back pressure on the bus if software supplies more data than can be consumed, or if software reads more than there is data available.
...
*  Multiple banks of flash memory (default to 2).
...
However, the system host is only able to access the [data partitions](./doc/theory_of_operation.md#host-and-protocol-controller-handling).
...
During TEST states, the isolated page is only programmable if:
* `lc_iso_part_sw_wr_en` is set, but `lc_iso_part_sw_rd_en` is not.

During PROD and RMA states, the isolated page is also readable if both:
* `lc_iso_part_sw_wr_en` and `lc_iso_part_sw_rd_en` are set.
