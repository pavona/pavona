...
The description is created in `.bzl` files using two macros provided by [`//rules/pavona:hw.bzl`](../../../rules/pavona/hw.bzl):
- `pavona_ip`: create the description of an IP,
- `pavona_top`: create the description of a top.
...
- A low-level API defined in `//rules/pavona:hw.bzl` which directly operates on the `pavona_top`-created objects.
...
- The most general macro is `pavona_select_top_attr(attr_name, required = True, default = None, fn = None)`.
...
- Sometimes, one only wants to test the existence of an attribute, in which case `pavona_if_top_attr(attr_name, obj, default)` is sufficient.
  It creates a select expression that evaluates to `obj` for top with the `attr_name` attribute, and to `default` for the others.
- It is sometimes necessary to mark target as compatible only with top providing a certain attribute. This can be done using `pavona_require_top_attr(attr_name)`.
- Finally, a very common case is when an attribute is simply a label string and we want to create an alias to this target.
  This can be done using `pavona_alias_top_attr(name, attr_name, required = True, default = None, fn = None)` which works similarly to `pavona_select_top_attr` but creates an `alias` target instead.

A similar set of API exists for IP attributes, see `pavona_select_ip_attr`, `pavona_if_ip_attr` and `pavona_require_ip_attr`.
...
cc_library(
    ...
    deps = [
        ...
        pavona_select_top_attr("ast_lib", required = False, default = ":ast_lib_stub"),
)
```
Note that in this particular case, since `ast_lib` is just a label-string, we can use the convenience macro `pavona_alias_top_attr` to create a target:
```py
# Alternative to the above for simple label-string attributes
load(
    "//hw/top:defs.bzl",
    "pavona_alias_top_attr",
)
