...
load("//rules/pavona:hw.bzl", "get_ip_attr", "get_top_attr", "has_ip_attr", "has_top_attr")
...
def pavona_if_ip(ip, obj, default):
    """
    ...
    return select({
        "@pavona_pavona//hw/top:is_{}".format(top): obj
        for top in compatible_tops
    } | {
        "//conditions:default": default,
    })

def pavona_require_ip(ip):
    """
    ...
    ```
    """
    return pavona_if_ip(ip, [], ["@platforms//:incompatible"])

def pavona_select_top(values, default):
    """
    ...
    for (tops, value) in values.items():
        if type(tops) == "string":
            tops = [tops]
        for top in tops:
            branches["@pavona_pavona//hw/top:is_{}".format(top)] = value
    branches["//conditions:default"] = default
    return select(branches)

def pavona_require_top(top):
    """
    Return a value that can be used with `target_compatible_with` to
    ...
    ```
    """
    return pavona_select_top({top: []}, ["@platforms//:incompatible"])

def pavona_select_top_attr(attr_name, required = True, default = None, fn = None):
    """
    ...
    branches = {
        "@pavona_pavona//hw/top:is_{}".format(top.name): maybe_fn(get_top_attr(top, attr_name))
        for top in ALL_TOPS
...
def pavona_if_top_attr(attr_name, obj, default):
    """
    ...
    ```
    """
    return pavona_select_top_attr(attr_name, fn = lambda x: obj, required = False, default = default)

def pavona_require_top_attr(attr_name):
    """
    ...
    """
    return pavona_if_top_attr(attr_name, [], ["@platforms//:incompatible"])

def pavona_alias_top_attr(name, attr_name, required = True, default = None, fn = None):
    """
    ...
    native.alias(
        name = name,
        actual = pavona_select_top_attr(attr_name, required, default, fn),
        target_compatible_with = pavona_require_top_attr(attr_name) if required else [],
    )

def pavona_select_ip_attr(ipname, attr_name, required = True, default = None, fn = None):
    """
    ...
    branches = {
        "@pavona_pavona//hw/top:is_{}".format(top.name): maybe_fn(get_ip_attr(top, ipname, attr_name))
        for top in ALL_TOPS
...
def pavona_if_ip_attr(ipname, attr_name, obj, default):
    """
    ...
    """
    return pavona_select_ip_attr(ipname, attr_name, fn = lambda x: obj, required = False, default = default)

def pavona_require_ip_attr(ipname, attr_name):
    """
    Return a select expression which can be used together with `target_compatible_with`
    to express the requirement that the top must have the required IP and the IP has
    the required attribute.
    """
    return pavona_if_ip_attr(ipname, attr_name, [], ["@platforms//:incompatible"])
