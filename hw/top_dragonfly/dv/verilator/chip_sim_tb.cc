...
int main(int argc, char **argv) {
  ...
  std::string top_scope(dut_scope + ".top_dragonfly");
  ...
  // in chip_dragonfly_verilator.sv.  It must be at least 2 cycles of clk_aon.
  simctrl.SetInitialResetDelay(20000);
  simctrl.SetResetDuration(10);

  std::cout << "Simulation of OpenTitan Dragonfly" << std::endl
            << "==================================" << std::endl
            << std::endl;
