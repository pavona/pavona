...
//
// The top_dragonfly SoC provides a bunch of fixed-function and muxed IO pads. The interface serves
...
`ifdef XCELIUM
  `define TOP_HIER          tb.dut.top_dragonfly
`else
  `define TOP_HIER          top_dragonfly
`endif
...
`define UART_HIER(i)        `TOP_HIER.u_uart``i
  ...
  wire [top_dragonfly_pkg::DioPadCount-1:0] dios;
  wire [top_dragonfly_pkg::MioPadCount-1:0] mios;

  // Functional interface: for testing ALL chip IOs, such as pinmux and padctrl tests.
  pins_if#(.Width(top_dragonfly_pkg::MioPadCount), .PullStrength("Weak")) mios_if(.pins(mios));
  pins_if#(.Width(top_dragonfly_pkg::DioPadCount), .PullStrength("Weak")) dios_if(.pins(dios));
  ...
  function automatic void cfg_default_weak_pulls_on_dios(bit enable);
    if (enable) begin
      ...
      dios_if.pins_pu[top_dragonfly_pkg::DioPadPorN] = 1;

      // No need of pulls for the SPI host peripheral.
      dios_if.pins_pd[top_dragonfly_pkg::DioPadSpiHostCsL:top_dragonfly_pkg::DioPadSpiHostD0]
          = '0;
    end else begin
      dios_if.disconnect();
    end
  ...
  for (genvar i = top_dragonfly_pkg::MioPadMio0; i < top_dragonfly_pkg::MioPadCount; i++)
  begin : gen_mios_x_check
    wire glitch_free_io;
    assign #1ps glitch_free_io = mios[i];

    top_dragonfly_pkg::mio_pad_e named_io = top_dragonfly_pkg::mio_pad_e'(i);
  ...
  pins_if #(.Width(1), .PullStrength("Weak")) por_n_if(.pins(dios[top_dragonfly_pkg::DioPadPorN]));

  // Functional (dedicated) interface (analog input): OTP ext volt.
  pins_if #(.Width(1), .PullStrength("Weak")) otp_ext_volt_if(
    .pins(dios[top_dragonfly_pkg::DioPadOtpExtVolt])
  );
  ...
  spi_if spi_host_if(
    ...
    .sio  (dios[top_dragonfly_pkg::DioPadSpiDevD3:top_dragonfly_pkg::DioPadSpiDevD0])
  );
  assign dios[top_dragonfly_pkg::DioPadSpiDevClk] = enable_spi_host | enable_spi_tpm ?
      spi_host_if.sck : 1'bz;
  assign dios[top_dragonfly_pkg::DioPadSpiDevCsL] = enable_spi_host ? spi_host_if.csb[0] : 1'bz;
  assign dios[top_dragonfly_pkg::DioPadSpiDevTpmCsL] = enable_spi_tpm ? spi_host_if.csb[1] : 1'bz;
  initial begin
  ...
  spi_if spi_device0_if(
    ...
    .sio  (dios[top_dragonfly_pkg::DioPadSpiHostD3:
                top_dragonfly_pkg::DioPadSpiHostD0])
  );

  assign spi_device0_if.sck = __enable_spi_device[0] ?
      dios[top_dragonfly_pkg::DioPadSpiHostClk] : 1'bz;

  assign spi_device0_if.csb = __enable_spi_device[0] ?
      {{NUM_CSB-1{1'b1}}, dios[top_dragonfly_pkg::DioPadSpiHostCsL]} : {NUM_CSB{1'b1}};
  ...
  pins_if #(.Width(3), .PullStrength("Weak")) sw_straps_if(
    .pins(dios[top_dragonfly_pkg::DioPadGpio24:top_dragonfly_pkg::DioPadGpio22])
  );
  ...
  pins_if #(.Width(NUM_GPIOS), .PullStrength("Weak")) gpios_if(
    .pins({dios[top_dragonfly_pkg::DioPadGpio31], dios[top_dragonfly_pkg::DioPadGpio30],
           dios[top_dragonfly_pkg::DioPadGpio29], dios[top_dragonfly_pkg::DioPadGpio28],
           dios[top_dragonfly_pkg::DioPadGpio27], dios[top_dragonfly_pkg::DioPadGpio26],
           dios[top_dragonfly_pkg::DioPadGpio25], dios[top_dragonfly_pkg::DioPadGpio24],
           dios[top_dragonfly_pkg::DioPadGpio23], dios[top_dragonfly_pkg::DioPadGpio22],
           dios[top_dragonfly_pkg::DioPadGpio21], dios[top_dragonfly_pkg::DioPadGpio20],
           dios[top_dragonfly_pkg::DioPadGpio19], dios[top_dragonfly_pkg::DioPadGpio18],
           dios[top_dragonfly_pkg::DioPadGpio17], dios[top_dragonfly_pkg::DioPadGpio16],
           dios[top_dragonfly_pkg::DioPadGpio15], dios[top_dragonfly_pkg::DioPadGpio14],
           dios[top_dragonfly_pkg::DioPadGpio13], dios[top_dragonfly_pkg::DioPadGpio12],
           dios[top_dragonfly_pkg::DioPadGpio11], dios[top_dragonfly_pkg::DioPadGpio10],
           dios[top_dragonfly_pkg::DioPadGpio9],  dios[top_dragonfly_pkg::DioPadGpio8],
           dios[top_dragonfly_pkg::DioPadGpio7],  dios[top_dragonfly_pkg::DioPadGpio6],
           dios[top_dragonfly_pkg::DioPadGpio5],  dios[top_dragonfly_pkg::DioPadGpio4],
           dios[top_dragonfly_pkg::DioPadGpio3],  dios[top_dragonfly_pkg::DioPadGpio2],
           dios[top_dragonfly_pkg::DioPadGpio1],  dios[top_dragonfly_pkg::DioPadGpio0]})
  );

  // Functional (dedicated) interface: JTAG (valid during debug enabled LC state only).
  // JTAG has dedicated pads for dragonfly, so leave it always enabled.
...
`endif
  ...
  assign dios[top_dragonfly_pkg::DioPadJtagTms] = __enable_jtag ? jtag_if.tms : 1'bz;
  assign jtag_if.tdo = __enable_jtag ? dios[top_dragonfly_pkg::DioPadJtagTdo] : 1'bz;
  assign dios[top_dragonfly_pkg::DioPadJtagTdi] = __enable_jtag ? jtag_if.tdi : 1'bz;
  assign dios[top_dragonfly_pkg::DioPadJtagTck] = __enable_jtag ? jtag_if.tck : 1'bz;
  assign dios[top_dragonfly_pkg::DioPadJtagTrstN] = __enable_jtag ? jtag_if.trst_n : 1'bz;

  function automatic void set_tdo_pull(bit value);
    if (value) begin
      dios_if.pins_pd[top_dragonfly_pkg::DioPadJtagTdo] = 0;
      dios_if.pins_pu[top_dragonfly_pkg::DioPadJtagTdo] = 1;
    end else begin
      dios_if.pins_pd[top_dragonfly_pkg::DioPadJtagTdo] = 1;
      dios_if.pins_pu[top_dragonfly_pkg::DioPadJtagTdo] = 0;
    end
  endfunction
  ...
  pins_if #(.Width(1), .PullStrength("Weak")) pinmux_wkup_if(
    .pins(dios[top_dragonfly_pkg::DioPadGpio0])
  );

  // Functional (dedicated) interface: UARTs.
  localparam int AssignedUartTxIos[NUM_UARTS] = {
      top_dragonfly_pkg::DioPadUartTx
  };
  localparam int AssignedUartRxIos[NUM_UARTS] = {
      top_dragonfly_pkg::DioPadUartRx
  };
  ...
  localparam int AssignedI2cSclIos [NUM_I2CS]  = {
    top_dragonfly_pkg::DioPadI2cScl
  };
  localparam int AssignedI2cSdaIos [NUM_I2CS] = {
    top_dragonfly_pkg::DioPadI2cSda
  };
  ...
  //
  // The TB currently instantiates 'chip_dragonfly_asic' as the DUT, but this does not
  ...
  mbx_if dragonfly_mbx_if(.clk(mbx_if_clk), .rst_n(mbx_if_rst_n));
  function automatic void connect_mbx_if();
    force dragonfly_mbx_if.interrupts    = mbx_interrupts;
    force dragonfly_mbx_if.mbx_tl_if.d2h = `TOP_HIER.mbx_tl_rsp_o;
    force `TOP_HIER.mbx_tl_req_i          = dragonfly_mbx_if.mbx_tl_if.h2d;
  endfunction
  function automatic void disconnect_mbx_if();
    release dragonfly_mbx_if.interrupts;
    release dragonfly_mbx_if.mbx_tl_if.d2h;
  ...
  clk_rst_if#("ExtClkDriver") ext_clk_if(
     .clk (mios[top_dragonfly_pkg::MioPadMio11]),
    .rst_n(dios[top_dragonfly_pkg::DioPadPorN])
  );
...
`endif
  ...
  initial begin
    ...
    uvm_config_db#(virtual tl_if)::set(
        null, "*.env.m_tl_agent_chip_soc_mbx_reg_block*", "vif", dragonfly_mbx_if.mbx_tl_if);
  ...
  // TODO: Autogen this in top_<top>_pkg.
  function automatic string get_hier_path(top_dragonfly_pkg::peripheral_e peripheral);
