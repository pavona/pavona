...
class chip_env_cfg #(type RAL_T = chip_ral_pkg::chip_reg_block) extends cip_base_env_cfg #(
  ...
  virtual function void initialize(bit [TL_AW-1:0] csr_base_addr = '1);
    ...
    debugger.num_triggers = 4;  // TODO: wire this from `top_dragonfly_pkg`.
    ...
    // the xbar fabric is itself quite complex for Dragonfly.
    ...
    foreach (sw_images[i]) begin
      if ("prebuilt" inside {sw_image_flags[i]}) begin
        ...
        // same image names apply.
          ...
          // If Test type (e.g., pavona_functest) and flash image was
          // generated by the `pavona_functest` Bazel macro itself, then we
          ...
          if ("ot_flash_binary" inside {sw_image_flags[i]} ||
              "new_rules" inside {sw_image_flags[i]}) begin
            sw_images[i] = $sformatf("%0s_%0s", sw_images[i], sw_build_device);
          // If Test type (e.g., pavona_functest) and flash image was by a
          // `pavona_flash_binary` macro, then no need to tweak the name.
          end else begin
          ...
          if ("signed" inside {sw_image_flags[i]}) begin
            // Options match DEFAULT_SIGNING_KEYS in `rules/pavona.bzl`.
