...
class chip_sw_power_virus_vseq extends chip_sw_base_vseq;
  ...
  virtual task configure_i2c_agents();
    ...
    // Hard-coded from `sw/top_dragonfly/sw/device/arch/device_sim_dv.c`.
  ...
  virtual task check_ip_activity();
    ...
    wait (cfg.chip_vif.dios[top_dragonfly_pkg::DioPadGpio0]);
  ...
  virtual task body();
    ...
    $assertoff(0, "tb.dut.top_dragonfly.u_spi_device.u_readcmd.u_readsram.u_sram_fifo.DataKnown_A");
    $assertoff(0, "tb.dut.top_dragonfly.u_spi_device.u_readcmd.u_readsram.u_fifo.DataKnown_A");
    super.body();
