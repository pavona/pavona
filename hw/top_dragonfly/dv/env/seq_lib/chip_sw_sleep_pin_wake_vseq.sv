...
// the interfaces are hard-coded. Refer tb/tb.sv chip_dragonfly_asic
// instantiation.
//
...
class chip_sw_sleep_pin_wake_vseq extends chip_sw_base_vseq;
  ...
  top_dragonfly_pkg::pad_type_e pad_type;
  ...
  virtual task body();
    ...
    pad_type = top_dragonfly_pkg::pad_type_e'(cfg.sw_logger_vif.printed_arg[0]);
    ...
    fork
      ...
      if (pad_type == top_dragonfly_pkg::DioPad &&
          pad_idx inside {[top_dragonfly_pkg::DioPadGpio24:
                           top_dragonfly_pkg::DioPadGpio22]}) begin
  ...
  function void drive_pad(top_dragonfly_pkg::pad_type_e pad_type, int unsigned pad_idx, logic value);
    `uvm_info(`gfn, $sformatf("Driving %0s[%0d] to %0b", pad_type.name(), pad_idx, value), UVM_LOW)
    case (pad_type)
      top_dragonfly_pkg::DioPad: begin
        `DV_CHECK(pad_idx inside {[0:top_dragonfly_pkg::DioCount-1]})
      ...
      top_dragonfly_pkg::MioPad: begin
