  ...
  virtual function void random_rom_init_with_digest();
    ...
    for (int addr = 0; addr < Rom0MaxCheckAddr; addr += TL_DW/8) begin
      ...
      rom0.rom_encrypt_write32_integ(addr,
                                     rnd_data,
                                     top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl0ScrKey,
                                     top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl0ScrNonce,
                                     1'b1); // Enable scrambling.
    end

    // Update the ROM digest.
    rom0.update_rom_digest(top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl0ScrKey,
                           top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl0ScrNonce);
    ...
    for (int addr = 0; addr < Rom1MaxCheckAddr; addr += TL_DW/8) begin
      ...
      rom1.rom_encrypt_write32_integ(addr,
                                     rnd_data,
                                     top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl1ScrKey,
                                     top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl1ScrNonce,
                                     1'b1); // Enable scrambling.
    end

    // Update the ROM digest.
    rom1.update_rom_digest(top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl1ScrKey,
                           top_dragonfly_rnd_cnst_pkg::RndCnstRomCtrl1ScrNonce);
  endfunction
  ...
  task test_mem_rw(uvm_mem mem, int max_access = 2048);
    ...
    for (int i = 0; i < max_access; ++i) begin
      ...
      if (mem.get_access() == "RW") begin
        ...
        if (byte_addr >= top_dragonfly_pkg::TOP_DRAGONFLY_ROM_CTRL0_ROM_BASE_ADDR &&
            byte_addr <  (top_dragonfly_pkg::TOP_DRAGONFLY_ROM_CTRL0_ROM_BASE_ADDR +
                          top_dragonfly_pkg::TOP_DRAGONFLY_ROM_CTRL0_ROM_SIZE_BYTES)) begin
