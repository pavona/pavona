...
class chip_sw_keymgr_dpe_key_derivation_vseq extends chip_sw_base_vseq;
  ...
  virtual function keymgr_dpe_pkg::keymgr_dpe_slot_t get_key_slot(int unsigned slot_idx);
    string path = $sformatf("tb.dut.top_dragonfly.u_keymgr_dpe.u_ctrl.key_slots_q[%0d]", slot_idx);
  ...
  virtual function adv_data_t get_creator_data();
    ...
    creator_data.HardwareRevisionSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeRevisionSeed;
    ...
    creator_data.HealthMeasurement =
        lc_at_prod ? top_dragonfly_rnd_cnst_pkg::RndCnstLcCtrlLcKeymgrDivProduction
                   : top_dragonfly_rnd_cnst_pkg::RndCnstLcCtrlLcKeymgrDivTestUnlocked;
  ...
  virtual function key_t get_otp_owner_seed();
    ...
    // TODO(#26288): Secret3 present only in Dragonfly.
  ...
  virtual function gen_data_t get_gen_data(tl_data_t key_version,
    ...
    unique case (dest)
      keymgr_pkg::None: begin // SW
        gen_data.HwDestSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeNoneSeed;
        gen_data.OutputSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeSoftOutputSeed;
      end

      keymgr_pkg::Aes: begin // HW -> AES
        gen_data.HwDestSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeAesSeed;
        gen_data.OutputSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeHardOutputSeed;
      end

      keymgr_pkg::Kmac: begin // HW -> KMAC
        gen_data.HwDestSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeKmacSeed;
        gen_data.OutputSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeHardOutputSeed;
      end

      keymgr_pkg::Acc: begin // HW -> ACC
        gen_data.HwDestSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeAccSeed;
        gen_data.OutputSeed = top_dragonfly_rnd_cnst_pkg::RndCnstKeymgrDpeHardOutputSeed;
      end
  ...
  virtual function key_shares_t get_output(keymgr_pkg::keymgr_key_dest_e dest);
    unique case (dest)
      ...
      keymgr_pkg::Aes:  return get_hw_output("tb.dut.top_dragonfly.u_keymgr_dpe.aes_key_o");
      keymgr_pkg::Kmac: return get_hw_output("tb.dut.top_dragonfly.u_keymgr_dpe.kmac_key_o");
  ...
  virtual function acc_key_shares_t get_output_acc();
    string path = "tb.dut.top_dragonfly.u_keymgr_dpe.acc_key_o";
