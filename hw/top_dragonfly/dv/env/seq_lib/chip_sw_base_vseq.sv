...
`endif
  ...
  virtual function void sw_symbol_backdoor_access(input string symbol,
                                                  inout bit [7:0] data[],
                                                  // Used to be SwTypeTestSlotA, but Dragonfly
  ...
  virtual task jtag_otp_program32(int addr,
    ...
    bit idle = 0;
    int base_addr = top_dragonfly_pkg::TOP_DRAGONFLY_OTP_CTRL_CORE_BASE_ADDR;
  ...
  virtual task enable_small_rma();
    string path = "tb.dut.top_dragonfly.u_flash_ctrl.u_flash_hw_if";
    string mypath;
