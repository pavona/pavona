...
class chip_sw_rom_e2e_jtag_debug_vseq extends chip_sw_base_vseq;
  ...
  virtual task body();
    ...
    // `pavona_gdb_fpga_cw310_test` target `rom_e2e_debug_test_otp__<lc_state>` in the bazel
  ...
  task test_chip_mem_access();
    ...
    addr_range_t sram_main = '{
        start_addr: top_dragonfly_pkg::TOP_DRAGONFLY_SRAM_CTRL_MAIN_RAM_BASE_ADDR,
        end_addr: top_dragonfly_pkg::TOP_DRAGONFLY_SRAM_CTRL_MAIN_RAM_BASE_ADDR +
                  top_dragonfly_pkg::TOP_DRAGONFLY_SRAM_CTRL_MAIN_RAM_SIZE_BYTES - 1

    };
    addr_range_t sram_ret = '{
        start_addr: top_dragonfly_pkg::TOP_DRAGONFLY_SRAM_CTRL_RET_AON_RAM_BASE_ADDR,
        end_addr: top_dragonfly_pkg::TOP_DRAGONFLY_SRAM_CTRL_RET_AON_RAM_BASE_ADDR +
                  top_dragonfly_pkg::TOP_DRAGONFLY_SRAM_CTRL_RET_AON_RAM_SIZE_BYTES - 1
