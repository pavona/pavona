...
static void init_peripherals(void) {
  ...
  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_ALERT_HANDLER_BASE_ADDR);
  CHECK_DIF_OK(dif_alert_handler_init(base_addr, &alert_handler));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_ACC_BASE_ADDR);
  CHECK_DIF_OK(dif_acc_init(base_addr, &acc));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_AES_BASE_ADDR);
  CHECK_DIF_OK(dif_aes_init(base_addr, &aes));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_AON_TIMER_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_aon_timer_init(base_addr, &aon_timer_aon));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_CLKMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_clkmgr_init(base_addr, &clkmgr_aon));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_CSRNG_BASE_ADDR);
  CHECK_DIF_OK(dif_csrng_init(base_addr, &csrng));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_DMA_BASE_ADDR);
  CHECK_DIF_OK(dif_dma_init(base_addr, &dma));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_EDN0_BASE_ADDR);
  CHECK_DIF_OK(dif_edn_init(base_addr, &edn0));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_EDN1_BASE_ADDR);
  CHECK_DIF_OK(dif_edn_init(base_addr, &edn1));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_ENTROPY_SRC_BASE_ADDR);
  CHECK_DIF_OK(dif_entropy_src_init(base_addr, &entropy_src));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_GPIO_BASE_ADDR);
  CHECK_DIF_OK(dif_gpio_init(base_addr, &gpio));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_HMAC_BASE_ADDR);
  CHECK_DIF_OK(dif_hmac_init(base_addr, &hmac));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_I2C0_BASE_ADDR);
  CHECK_DIF_OK(dif_i2c_init(base_addr, &i2c0));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_KEYMGR_DPE_BASE_ADDR);
  CHECK_DIF_OK(dif_keymgr_dpe_init(base_addr, &keymgr_dpe));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_KMAC_BASE_ADDR);
  CHECK_DIF_OK(dif_kmac_init(base_addr, &kmac));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_LC_CTRL_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_lc_ctrl_init(base_addr, &lc_ctrl));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX0_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx0));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX1_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx1));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX2_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx2));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX3_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx3));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX4_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx4));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX5_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx5));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX6_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx6));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX_JTAG_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx_jtag));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX_PCIE0_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx_pcie0));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_MBX_PCIE1_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx_pcie1));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_OTP_CTRL_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_otp_ctrl_init(base_addr, &otp_ctrl));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_PINMUX_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_pinmux_init(base_addr, &pinmux_aon));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_PWRMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_pwrmgr_init(base_addr, &pwrmgr_aon));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_ROM_CTRL0_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_rom_ctrl_init(base_addr, &rom_ctrl0));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_ROM_CTRL1_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_rom_ctrl_init(base_addr, &rom_ctrl1));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_RSTMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_rstmgr_init(base_addr, &rstmgr_aon));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_RV_CORE_IBEX_CFG_BASE_ADDR);
  CHECK_DIF_OK(dif_rv_core_ibex_init(base_addr, &rv_core_ibex));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_RV_PLIC_BASE_ADDR);
  CHECK_DIF_OK(dif_rv_plic_init(base_addr, &rv_plic));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_RV_TIMER_BASE_ADDR);
  CHECK_DIF_OK(dif_rv_timer_init(base_addr, &rv_timer));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SOC_DBG_CTRL_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_soc_dbg_ctrl_init(base_addr, &soc_dbg_ctrl));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SOC_PROXY_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_soc_proxy_init(base_addr, &soc_proxy));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SPI_DEVICE_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_device_init(base_addr, &spi_device));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SPI_HOST0_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_host_init(base_addr, &spi_host0));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SRAM_CTRL_MAIN_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_sram_ctrl_init(base_addr, &sram_ctrl_main));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SRAM_CTRL_MBOX_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_sram_ctrl_init(base_addr, &sram_ctrl_mbox));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_SRAM_CTRL_RET_AON_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_sram_ctrl_init(base_addr, &sram_ctrl_ret_aon));

  base_addr = mmio_region_from_addr(TOP_DRAGONFLY_UART0_BASE_ADDR);
...
static void trigger_alert_test(void) {
  ...
  for (dif_acc_alert_t i = 0; i < 2; ++i) {
    ...
    exp_alert = kTopDragonflyAlertIdAccFatal + i;
  ...
  for (dif_aes_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdAesRecovCtrlUpdateErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_aon_timer_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdAonTimerAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_clkmgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdClkmgrAonRecovFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_csrng_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdCsrngRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_dma_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdDmaFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_edn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdEdn0RecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_edn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdEdn1RecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_entropy_src_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdEntropySrcRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_gpio_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdGpioFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_hmac_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdHmacFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_i2c_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdI2c0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_keymgr_dpe_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdKeymgrDpeRecovOperationErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_kmac_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdKmacRecovOperationErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_lc_ctrl_alert_t i = 0; i < 3; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdLcCtrlFatalProgError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx2FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx3FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx4FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx5FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbx6FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbxJtagFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbxPcie0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdMbxPcie1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  if (kBootStage != kBootStageOwner) {
    ...
    for (dif_otp_ctrl_alert_t i = 0; i < 5; ++i) {
      ...
      // Verify that alert handler received it.
      exp_alert = kTopDragonflyAlertIdOtpCtrlFatalMacroError + i;
      CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
          &alert_handler, exp_alert, &is_cause));
      CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pinmux_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdPinmuxAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pwrmgr_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdPwrmgrAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rom_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdRomCtrl0Fatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rom_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdRomCtrl1Fatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rstmgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdRstmgrAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_core_ibex_alert_t i = 0; i < 4; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdRvCoreIbexFatalSwErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_plic_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdRvPlicFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_timer_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdRvTimerFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_soc_dbg_ctrl_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSocDbgCtrlFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_soc_proxy_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSocProxyFatalAlertIntg + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_device_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSpiDeviceFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_host_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSpiHost0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSramCtrlMainFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSramCtrlMboxFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdSramCtrlRetAonFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_uart_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDragonflyAlertIdUart0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
