...
Infrastructure signals such as clocks, resets and the entropy source are provided by the analog sensor top (AST) block, which is connected to the Dragonfly-internal power, clock and reset manager blocks.
      ...
      <ul>
        ...
        <li>Security peripherals:
          <ul>
            ...
            <li>Programmable big number accelerator for RSA and ECC (ACC)</li>
...
## Discrete Egret Differences

The Dragonfly configuration derived from the discrete "Egret" has been extended to meet the requirements for an SoC-integrated RoT.
...
- An [access control range check module](../ip_autogen/ac_range_check/README.md) that ensures that Dragonfly can access only authorized addresses in the SoC
- A [register-access control list controller](../ip_autogen/racl_ctrl/README.md) that defines role-based access permissions to Dragonfly's registers
