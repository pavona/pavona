...
module chip_dragonfly_verilator #(
  ...
  // and as a result all the clk_aon logic inside top_dragonfly does not
  // get reset
  prim_clock_div #(
    .Divisor(4)
  ) u_aon_div (
  ...
  always_comb begin
    ...
    if ((ctn_sm1_to_s1n_tl_h2d.a_address & ~(TOP_DRAGONFLY_SOC_PROXY_RAM_CTN_SIZE_BYTES-1)) ==
        (TOP_DRAGONFLY_SOC_PROXY_RAM_CTN_BASE_ADDR - TOP_DRAGONFLY_SOC_PROXY_CTN_BASE_ADDR)) begin
  ...
  top_dragonfly #(
    ...
    .acc_imem_ram_1p_cfg_i                    ( '0 ),
    .acc_imem_ram_1p_cfg_rsp_o                (    ),
    .acc_dmem_ram_1p_cfg_i                    ( '0 ),
    .acc_dmem_ram_1p_cfg_rsp_o                (    ),
...
endmodule : chip_dragonfly_verilator
