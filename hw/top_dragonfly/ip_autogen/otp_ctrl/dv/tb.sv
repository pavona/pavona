...
module tb;
  ...
  wire otp_ctrl_pkg::acc_otp_key_req_t  acc_req;
  wire otp_ctrl_pkg::acc_otp_key_rsp_t  acc_rsp;
  ...
  push_pull_if #(.DeviceDataWidth(ACC_DATA_SIZE)) acc_if(.clk(clk), .rst_n(rst_n));
  ...
  assign otp_ctrl_if.acc_ack  = acc_if.ack;
  ...
  // dut
  otp_ctrl #(
    .RndCnstScrmblKey0(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlScrmblKey0),
    .RndCnstScrmblKey1(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlScrmblKey1),
    .RndCnstScrmblKey2(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlScrmblKey2),
    .RndCnstScrmblKey3(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlScrmblKey3),
    .RndCnstDigestConst0(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlDigestConst0),
    .RndCnstDigestConst1(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlDigestConst1),
    .RndCnstDigestIV0(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlDigestIV0),
    .RndCnstDigestIV1(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlDigestIV1),
    .RndCnstPartInvDefault(top_dragonfly_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault)
  ) dut (
    .clk_i                      (clk        ),
    .rst_ni                     (rst_n      ),
    // edn
    ...
    // acc
    .acc_otp_key_i             (acc_req),
    .acc_otp_key_o             (acc_rsp),
...
//    .MemInitFile(),
  ...
  assign acc_req       = acc_if.req;
  assign acc_if.ack    = acc_rsp.ack;
  assign acc_if.d_data = {acc_rsp.key, acc_rsp.nonce, acc_rsp.seed_valid};
  ...
  initial begin
    ...
    uvm_config_db#(virtual tl_if)::set(null, "*.env.m_tl_agent_otp_macro_prim_reg_block",
                                       "vif", prim_tl_if);
    uvm_config_db#(virtual push_pull_if#(.DeviceDataWidth(ACC_DATA_SIZE)))::set(null,
                   "*env.m_acc_pull_agent*", "vif", acc_if);
