...
The integrated Dragonfly top is a root of trust for measurement, integrated within the SoC.
...
- Only require minimal flow to enable a path to external flash that is under Dragonfly's control.

Flows involving reset and bring up for the rest of the SoC, which may be potentially more complex and involve proprietary code, are expected to be handled by other computing element(s) that are bootstrapped by the integrated Dragonfly and running authenticated / measured code.

This proposal introduces a second Dragonfly boot ROM partition with efuse/OTP based patching mechanism that enables each SoC to add such small customizations and provide critical survivability hooks while also removing the dependency on the integrated flash component.
...
![Integrated Dragonfly and Boot Building Blocks](building-blocks.svg)
...
- Base ROM design is common across all integrated Dragonfly instances.
...
- Supports security hardened functions such as signature verification, verification & loading of second ROM partition patch, other hardened crypto functions required at boot etc.
  Integrated Dragonfly reuses the majority of current base ROM implementation.
...
- Used to hold the patch code to be executed upon redirection.
  This can either be a dedicated SRAM or part of the main SRAM (Dragonfly currently implements the latter).
...
- Reset is deasserted to the SoC.
  Dragonfly is the first entity within the SoC to come out of reset and coordinates the release of reset & boot of other hardware blocks and programmable engines in the right order to expand the boot chain of trust.
...
- Second ROM partition execution starts (covered by OTP based patch code survivability mechanism).
  ...
  - If additional configuration is required, these steps are completed prior to accessing flash.
    ...
    *A few points to note in this regard*:
    ...
    - Integrated Dragonfly is the root of trust for measurement.
    ...
    - The external flash, if requiring additional boot path configuration by measured SoC agents, is under exclusive control of Dragonfly once initial boot path configuration is complete.
...
[^multi-roms]: This document does not *mandate* that all additional ROMs / FW images are measured by Dragonfly.
  Whether or not that is the case is outside the scope of this document.

[^ext-flash]: This is aligned with the *first admissible architecture for Dragonfly Integrated* (TODO: Not currently linked), which has a dedicated flash controller that is exclusively under Dragonfly's control.
...
- Load From external flash
  ...
  - Each SoC / product may have a custom QSPI flash layout.
    ...
    - A separate document shall outline the various use cases and requirements around the QSPI flash layout.
      ...
      - The first ROM partition is standardized over all Dragonfly implementations.

      - The second ROM partition may be vendor-specific, but Dragonfly will provide an open-source reference implementation.
...
- Reset of the SoC reset release and more complex & potentially some proprietary bring up mechanism shall be the responsibility of other SoC programmable agents (called as the SoC reset controller from here on) running verified code.
  ...
  - SoC reset controller may leverage Dragonfly's help to fetch, verify and load additional firmware packages for various programmable engines at different stages of the SoC specific reset release process.

  - Dragonfly and the SoC reset controller may use the DOE mailbox mechanism to coordinate the reset handshakes.
