...
static const top_scafi_deprecated_plic_target_t kHart = kTopScafi_deprecatedPlicTargetIbex0;
 ...
 * as the ISR.
 */
static volatile top_scafi_deprecated_plic_peripheral_t peripheral_expected;
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  top_scafi_deprecated_plic_peripheral_t peripheral = (top_scafi_deprecated_plic_peripheral_t)
      top_scafi_deprecated_plic_interrupt_for_peripheral[plic_irq_id];
  ...
  switch (peripheral) {
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralAonTimerAon: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralFlashCtrl: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralGpio: {
      dif_gpio_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralPwrmgrAon: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralSpiDevice: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralSpiHost0: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralUart0: {
      dif_uart_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralUart1: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
    case kTopScafiDeprecatedPlicPeripheralUsbdev: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_AON_TIMER_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_aon_timer_init(base_addr, &aon_timer_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_FLASH_CTRL_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_flash_ctrl_init(base_addr, &flash_ctrl));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_GPIO_BASE_ADDR);
  CHECK_DIF_OK(dif_gpio_init(base_addr, &gpio));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_PWRMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_pwrmgr_init(base_addr, &pwrmgr_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_SPI_DEVICE_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_device_init(base_addr, &spi_device));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_SPI_HOST0_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_host_init(base_addr, &spi_host0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_UART0_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_UART1_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_USBDEV_BASE_ADDR);
  CHECK_DIF_OK(dif_usbdev_init(base_addr, &usbdev));
#endif

  base_addr = mmio_region_from_addr(TOP_SCAFI_DEPRECATED_RV_PLIC_BASE_ADDR);
...
static void peripheral_irqs_trigger(void) {
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopScafiDeprecatedPlicPeripheralAonTimerAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralFlashCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralGpio;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralPwrmgrAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralSpiDevice;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralSpiHost0;
  status_default_mask = 0x0;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopScafiDeprecatedPlicPeripheralUart0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralUart1;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopScafiDeprecatedPlicPeripheralUsbdev;
  status_default_mask = 0x0;
...
bool test_main(void) {
  ...
  rv_plic_testutils_irq_range_enable(
      &plic, kHart, kTopScafi_deprecatedPlicIrqIdNone + 1, kTopScafi_deprecatedPlicIrqIdLast);
