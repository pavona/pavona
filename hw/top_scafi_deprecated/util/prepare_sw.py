...
"""
Script to patch the OpenTitan tree for building scafi_deprecated-friendly
software.
...
def main():
    parser = argparse.ArgumentParser(
        prog="prepare_sw",
        description="Script to prepare SW sources for Scafi_Deprecated",
    ...
    # selected non-egret top. These might be stale and confuse topgen.
    ...
    #    Specifically, we overwrite the egret autogen-ed files with the
    #    scafi_deprecated equivalents where they differ (i.e. any needed ip in
    #    `hw/topname/{ip, ip_autogen}`).
    # 2. SW sources currently contain
    #    hardcoded references to top_egret. We
    ...
    for reg_file in REG_FILES:
        ...
        dst = REPO_TOP / 'hw' / 'top_egret' / reg_file
        print(f"* Copying {src} -> {dst}")
        dst.write_text(src.read_text())

    for suffix in ['.c', '.h', '_memory.h', '_memory.ld']:
        old = REPO_TOP / 'hw' / topname / 'sw/autogen' / (topname + suffix)
        new = REPO_TOP / 'hw/top_egret/sw/autogen' / ('top_egret' + suffix)
        print(f"* {old} -> {new}")

        text = old.read_text()
        text = text.replace(name, 'egret')
        text = text.replace(name.capitalize(), 'Egret')
        text = text.replace(name.upper(), 'EGRET')

        # The SW build expects to find this file both in the top_egret dir AND
        # in the top_scafi_deprecated dir.
        new.write_text(text)
        (old.parent / new.name).write_text(text)

    # Hack to make bindgen generate less for Scafi_Deprecated.
    new = REPO_TOP / 'sw/host/ot_hal/bindgen/BUILD'
    text = new.read_text()
    text = text.replace('_TOPLEVEL = "egret"', '_TOPLEVEL = "scafi_deprecated"')
    new.write_text(text)
    ...
    shell_out([
        ...
        '--copt=-DOT_IS_SCAFI_DEPRECATED_REDUCED_SUPPORT_FOR_INTERNAL_USE_ONLY_',
