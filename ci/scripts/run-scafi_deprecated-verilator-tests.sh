...
# Only a single test is supported on Scafi_Deprecated (SD).
# Currently, Bazel cannot build the SD Verilator model, so we only build the test with Bazel and then use opentitantool directly
# SD Verilator model is built in a previous CI step
...
# Build the modified SD software.
./hw/top_scafi_deprecated/util/prepare_sw.py -b

# Build some other dependencies.
./bazelisk.sh build  \
    --copt=-DOT_IS_SCAFI_DEPRECATED_REDUCED_SUPPORT_FOR_INTERNAL_USE_ONLY_ \
    --features=-rv32_bitmanip \
    //sw/host/opentitantool //hw/top_egret/data/otp:img_rma
...
bazel-bin/sw/host/opentitantool/opentitantool \
    ...
    --logging=info \
    --interface=verilator \
    --verilator-bin=$BIN_DIR/hw/top_scafi_deprecated/Vchip_scafi_deprecated_verilator \
