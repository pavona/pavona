...
def top_variant(parts) -> str | None:
    '''Return the top name (minus the 'top_' prefix) from a path's parts, or
    None if the path is not under a hw/top_* directory.'''
    for p in parts:
...
def check_sim_cfg_files(grouped: dict) -> bool:
    ...
    for directory, files in grouped.items():
        ...
        if len(sim_cfg_files) == 1:
            ...
            if 'name' not in fields:
                ...
                if 'variant' in fields:
                    ...
                    # A single sim_cfg may carry a 'variant' only when it is a
                    # generated file: ip_autogen IPs or xbar cfgs generated by
                    # tlgen under a 'dv/autogen' directory. It is still named
                    # {name}_sim_cfg.hjson, and its variant must match the top.
                    if 'ip_autogen' in parts or 'autogen' in parts:
                        expected = f"{fields['name']}_sim_cfg.hjson"
                        if file != expected:
                            mismatches.append(f"Filename {directory}/{file}"
                                              f" - should be named {expected}")
                        top = top_variant(parts)
                        if top is None:
                            mismatches.append(f"Filename {directory}/{file}"
                                              f" - cannot determine top to validate 'variant'")
                        elif fields['variant'] != top:
                            mismatches.append(f"Filename {directory}/{file}"
                                              f" - 'variant' is {fields['variant']!r}, "
                                              f"should match top {top!r}")
                    else:
                        mismatches.append(f"Filename {directory}/{file}"
        ...
        # CASE 2: There are multiple sim_cfg files in the directory
            ...
            if has_variant:
                # A generated (ip_autogen) IP must use a single merged
                # {name}_sim_cfg.hjson carrying 'variant' (CASE 1), not a
                # base+variant split.
                if 'ip_autogen' in Path(directory).parts:
                    for file, _ in sim_cfg_files:
                        mismatches.append(f"Filename {directory}/{file}"
                                          f" - ip_autogen IP must use a single "
                                          f"{{name}}_sim_cfg.hjson with a 'variant' "
                                          f"field, not a base+variant split")
                    continue
        ...
        for file, fields in sim_cfgs_files:
            ...
            if dir_path == SEARCH_ROOT / "hw" / "dv":
                # all_sim_cfgs.hjson is the canonical batch; other batch cfgs
                # under hw/dv/ are named extensions of it, named after their own
                # 'name' field ({name}_sim_cfgs.hjson).
                if 'name' not in fields:
                    mismatches.append(f"Filename {directory}/{file}"
                                      f" should contain field 'name'")
                elif file != "all_sim_cfgs.hjson":
                    expected = f"{fields['name']}_sim_cfgs.hjson"
                    if file != expected:
                        mismatches.append(f"Filename {directory}/{file}"
                                          f" should be named {expected}")
    ...
    # There must always be a top-level all_sim_cfgs.hjson.
    all_cfgs = SEARCH_ROOT / "hw" / "dv" / "all_sim_cfgs.hjson"
    if not all_cfgs.exists():
        mismatches.append(f"Missing required {all_cfgs}")
