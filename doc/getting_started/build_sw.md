...
There are two categories of Pavona tests Bazel can build and run:
1. on-host, and
1. on-device.

On-host tests are compiled and run on the host machine, while on-device tests are compiled and run on (simulated/emulated) hardware.
...
* any test for host software, such as `opentitanlib`.
...
* `cw310`, `cw310_test_rom`, and `cw310_rom_with_fake_keys` are used to tag tests that depend on a correctly setup CW310 "Bergen Board" to emulate the specified hardware.
...
* `verilator` is used to tag tests that depend on a verilated model of the hardware (which can take a significant amount of time to build).
...
As described in the [software documentation](../../sw/README.md), there are three categories of Pavona software, all of which are built with Bazel. These include:
1. _device_ software,
1. _ACC_ software,
1. _host_ software,
...
Device software is built and run on hardware.
There are three "devices" for simulating/emulating hardware:
...
ACC programs use a specialized build flow (defined in `rules/acc.bzl`).
ACC programs produce the following artifacts:
...
* `<target>.rv32embed.{a,o}`: artifacts representing an ACC binary, set up to be linked into a RISC-V program

In terms of Bazel rules:
* the `acc_library` rule runs the assembler to create `<target>.o` artifacts, and
* the `acc_binary` and `acc_sim_test` rules run the linker on one or more `.o` files to create the `.elf` and `.rv32embed.{a,o}` artifacts.

Since ACC has limited instruction memory, the best practice is to list each file individually as an `acc_library`.
This way, binary targets can easily include only the files they need.

ACC programs run on the ACC coprocessor, unlike standard "on-device" programs that run on the main processor (Ibex).
There are two ways to run an ACC program:
1. Run a standalone binary (`.elf`) on the specialized ACC simulator.
