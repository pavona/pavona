...
To use an FPGA with Pavona you need two things:
...
To instantiate hardware on an FPGA you will need a bitstream.
...
If you are using the ChipWhisperer CW340 board with the Xilinx XCKU095-1FFVA1156C Kintex UltraScale or the CW310 board with the Xilinx Kintex 7 XC7K410T FPGA, you can download the latest passing pre-built bitstream from our public bitstream cache GCS bucket.
...
Pavona's build system automatically fetches pre-built bitstreams via the `@bitstreams` repository.
...
Specifically, you can build the [`//hw/bitstream/universal:splice`](../../hw/bitstream/universal/BUILD) Bazel target and specify any combination of:
...
While Bazel is the entry point for kicking off the FPGA synthesis, under the hood, it invokes FuseSoC, the hardware package manager / build system.
...
If you have a HyperDebug board, you can connect it to your CW340 board to enable executing more advanced test cases (such as test cases that drive various communication peripherals).
...
1. Ensure the CW340 FPGA base board is:
    1. Powered off (the *Control Power* switch in the top left corner, *SW7*, is set to the right).
...
Alternatively, for software build targets defined in Bazel BUILD files using the `pavona_binary` Bazel macro, you will use the former (**manual**) approach.
...
* bootstrap the desired software test image into the embedded flash.
...
Some on-device software targets are defined using the custom `pavona_binary` Bazel macro.
Unlike the `pavona_test` macro, the `pavona_binary` macro does **not** instantiate any Bazel test rules under the hood.
Therefore, to run such software on FPGA hardware, both a bitstream and the software target must be loaded manually onto the FPGA.
Below, we describe how to accomplish this, and in doing so, we shed some light on the tasks that Bazel automates through the use of `pavona_test` Bazel rules.
...
The `hello_world` demo software shows off some capabilities of the hardware.
...
Depending on the adapter's default state, the hardware may be held in reset when the adapter is initially connected.
...
For example, if you followed the [Getting Started](README.md) instructions, then make sure `/tools/riscv/bin` is on your `PATH`.
