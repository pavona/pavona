...
Various components inside a hardware Root-of-Trust (RoT) operate on sensitive assets such as identities and key material.
...
i) different flip-flop types for registering such sensitive assets inside hardware blocks incl. possible vulnerabilities,
ii) the interplay with other countermeasures and security hardening schemes, and
iii) why we **do not give a general recommendation for either flip-flop type** for sensitive assets in the earlier hardware design stages of comportable IP.
...
A specific example where it has been decided to use non-reset flops is discussed in the [appendix](#appendix-acc---a-case-for-non-reset-flops).

## Flip-Flop Types

At register-transfer level (RTL), there are two different flip-flop (flop) types for implementing registers: Reset Flops[^1] and Non-Reset Flops.
...
Obviously the implemented countermeasures have a big impact on the susceptibility to various attack types and therefore must be known when weighing up the risks of non-reset vs. reset flops for the considered hardware block.
...
For this reason, **we do not give a general recommendation for either flip-flop type** for sensitive assets in the earlier hardware design stages of hardware blocks.
Instead, it is **recommended** to take this decision later in the design process on a sub-block level in consideration of all countermeasures and security hardening employed.

## Appendix: ACC - a Case for Non-Reset Flops

The [Asymmetric Cryptographic Coprocessor (ACC)](../../../../hw/ip/acc/README.md) is a coprocessor for asymmetric cryptographic operations like RSA or elliptic curve cryptography (ECC).
...
At the same time, ACC implements a secure wipe mechanism to ensure that no register and memory contents survive a chip reset.
Moreover, several hardware countermeasures are in place to allow entering scan mode only in the TEST and RMA life-cycle states, in which no device root secrets are in the device yet or any more, respectively.

For these reasons, it has been decided to switch to non-reset flops for the general and wide data register files in ACC when approaching the D2S milestone.

<!-- Footnotes themselves at the bottom. -->

## Notes

[^1]: Inside Pavona's hardware blocks, all reset flops use an asynchronous, active-low reset with synchronous release.

[^2]: This is typically done when employing masking in software, e.g., inside the Asymmetric Cryptographic Coprocessor.
