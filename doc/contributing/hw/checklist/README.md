...
This document explains the recommended checklist items to review when transitioning from one [Development Stage](../development_stages.md) to another, for design, verification, and [software device interface function (DIF)](../../sw/device_interface_functions.md) stages.
...
The unit `.sv` exists and meets [comportability](../comportability/README.md) requirements.
...
Any integration specifics that are not captured by the [comportability](../comportability/README.md) specification have been documented.
...
The IP block conforms to the [style guide regarding X usage](../../style_guides/verilog_coding_style.md#dont-cares-xs).
...
For a full list of permitted asset and countermeasure types, see the [countermeasure.py](../../../../util/reggen/countermeasure.py) script that implements the name checks.
...
Implementations must follow the [Secure Hardware Design Guidelines](../../../security/implementation_guidelines/hardware/README.md).

In particular, note that:

- For duplicated counters [`prim_count`](../../../../hw/ip/prim/rtl/prim_count.sv) must be used.
- For duplicated LFSRs [`prim_double_lfsr`](../../../../hw/ip/prim/rtl/prim_double_lfsr.sv) must be used.
- For redundantly encoded FSMs, [the sparse-fsm-encode.py script](../../../../util/design/sparse-fsm-encode.py) must be used to generate the encoding (in conjunction with the [`PRIM_FLOP_SPARSE_FSM`](../../../../hw/ip/prim/rtl/prim_flop_macros.sv)) macro.
- For multibit signals, the `mubi` types in [`prim_mubi_pkg`](../../../../hw/ip/prim/rtl/prim_mubi_pkg.sv) should be used if possible.
...
Default random seeds and permutations for LFSRs can be generated with [the gen-lfsr-seed.py script](../../../../util/design/gen-lfsr-seed.py).
...
A security countermeasures testplan can be auto-generated (the first time) by the `reggen` tool for each DUT, and is placed at `hw/ip/<ip>/data/<ip>_sec_cm_testplan.hjson` (or equivalent).
...
Make sure to import the applicable common sec_cm [tests](../../../../hw/dv/tools/dvsim/tests) and [testplans](../../../../hw/dv/tools/dvsim/testplans).

### FPV_SEC_CM_PROVEN

All security countermeasure assertions are proven in FPV.
The required assertions for countermeasure are defined in [Security Countermeasure Verification Framework](../../dv/sec_cm_dv_framework/README.md).
...
[X-propagation](../../dv/methodology/README.md#x-propagation-xprop) is enabled in DV simulations.
...
This test should exercise the main datapath of the hardware module via the DIF, and should be able to be run on at least one platform (either on FPGA or in simulation).
...
The DIF follows DIF-specific guidelines in [`sw/device/lib/dif`](../../../../sw/device/lib/dif/README.md) and the C style guidelines.
