...
## `pavona_binary` Artifacts

- Query the locations of all Bazel-built artifacts for all Pavona devices for an `pavona_binary` macro:
  ```sh
  ./bazelisk.sh outquery-all <target>
  ```

> See [Pavona binary and test rules](../../rules/pavona/README.md)) for more details on the `pavona_binary` rule.

See [Building (and Testing) Software](../getting_started/build_sw.md#device-artifacts), device software can be built for multiple Pavona devices and memories, using Pavona-specific Bazel macros.

## `pavona_test` Artifacts

As described [Building (and Testing) Software](../getting_started/build_sw.md#device-artifacts), device software can be built for multiple Pavona devices and memories, using Pavona-specific Bazel macros.
Since running tests on multiple Pavona devices (whether DV or Verilator simulation, or an FPGA) involves building several software images for multiple memories, we provide a Bazel macro for this called `pavona_test`.
...
The Pavona supported linter for C/C++ files is `clang-format`, and can be run with Bazel as shown below.
...
`rustfmt` is used to format Rust code, and can be run with Bazel as shown below.
...
`buildifier` is used to automatically format Bazel files.
...
The Pavona supported linter for Bazel files is `buildifier`, and can be run with Bazel as shown below.
...
Note, to run (software) tests on these Pavona hardware platforms **does not** require these Bazel commands be invoked before the test commands above.
...
See the [`.bazelrc`](../../.bazelrc) file in the Pavona repository for more examples.
...
In addition to each user's `$HOME/.bazelrc` file and the project-level configurations in [`$REPO_TOP/.bazelrc`](../../.bazelrc), there is the option to add site-specific configuration options, by adding them to the file `$REPO_TOP/.bazelrc-site`.
...
However the "$HOME/.bazelrc" file is not specific to Pavona, which could create confusion for users working on a number of projects.

The policies and paths for each contributing site could vary greatly.
So in order to avoid accidental conflicts between sites, the `.bazelrc-site` file is explicitly _not_ included in the git repository and Pavona CI scripts will reject any commit that includes a `.bazelrc-site` file.
...
For example add the following line to your crontab (using `crontab -e`) to delete all files that were last accessed over 60 days ago.
