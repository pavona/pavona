...
This document provides an overview of the pavona repository directory structure.
...
│
├──ip_templates   => Templatized implementations of IP blocks whose
│                    full descriptions depend on the SoC using it
│
├──lint           => Scripts to run the `lint` tool on all RTL sources
│
├──top_foo        => An implementation of some Pavona SoC built using above
...
```
   ...
   │
   ├──ip                => IPs tailor-made for top_foo
   │  │
   │  ├──xbar           => XBAR implementation for top_foo
   │  │  ├──dv          => DV sources
   │  │  │  └──autogen  => auto-generated XBAR DV sources
   │  │  └──rtl         => RTL sources
   │  │     └──autogen  => Auto-generated XBAR RTL sources
   │  │
   │  ├──...            => More such IPs tailored for top_foo...
   │
   ├──ip_autogen        => top_foo specific IPs autogenerated from
   │                       hw/ip_templates
   │
...
```
sw
├──device                 => Sources compiled for the Pavona chip,
