...
secp256k1_verify:
  ...
  /* load 3*b for the complete a=0 addition formula.
     w27 <= b3 = dmem[secp256k1_b3] */
  li        x2, 27
  la        x3, secp256k1_b3
  bn.lid    x2, 0(x3)

  /* setup modulus n (curve order) and Barrett constant
     MOD <= w29 <= n = dmem[secp256k1_n]; w28 <= u_n = dmem[secp256k1_u_n]  */
  li        x2, 29
  la        x3, secp256k1_n
  bn.lid    x2, 0(x3)
  bn.wsrw   MOD, w29
  ...
  csrrs     x2, FG0, x0
  andi      x2, x2, 8
  bne       x2, x0, secp256k1_invalid_input

  /* Fail if w0 >= w29 <=> s >= n */
  bn.cmp    w0, w29
  csrrs     x2, FG0, x0
  andi      x2, x2, 1
  beq       x2, x0, secp256k1_invalid_input
  ...
  loopi     256, 38
    ...
    /* load base point x-coordinate
      w8 <= g_x = dmem [secp256k1_gx]; w9 <= g_y = dmem[secp256k1_gy] */
    bn.lid    x13, 0(x23)
    bn.lid    x14, 0(x24)
