...
.globl secp256k1_isoncurve
.globl secp256k1_check_public_key
.globl secp256k1_invalid_input
...
/**
 * Checks if a point is a valid curve point on curve secp256k1
 *
 * Returns rhs = x^3 + b  mod p
 *     and lhs = y^2  mod p
 ...
 * for the secp256k1 curve (so the ax term vanishes) and that b = 7 fits an
 * immediate, hence the routine is limited to secp256k1.
 ...
 * @param[out]     w18: lhs, left side of equation = (x^3 + b) mod p
 * @param[out]     w19: rhs, right side of equation = y^2 mod p
 *
 * clobbered registers: x2, x3, w18 to w29
...
secp256k1_isoncurve:
  ...
  /* Materialize the domain parameter b.
     w27 <= b = 7 */
  bn.addi   w27, w31, 7
  ...
  /* for curve secp256k1, 'a' is zero, therefore the ax term is omitted.
     w18 <= x^3 + b mod p = w19 + w27 mod p = lhs */
  bn.addm   w18, w19, w27
 ...
 *
 * clobbered registers: x2, x3, w2, w18 to w29
 * clobbered flag groups: FG0
 */
secp256k1_check_public_key:
  /* Init all-zero register. */
  bn.xor   w31, w31, w31

  /* Load domain parameter p.
       w29 <= dmem[secp256k1_p] = p */
  li        x2, 29
  la        x3, secp256k1_p
  bn.lid    x2, 0(x3)
  ...
  andi      x2, x2, 1
  bne       x2, x0, _x_valid
  jal       x0, secp256k1_invalid_input
  unimp
