...
def gen_secaddmodq_bc22_mlkem_test(
    ...
    for i in range(nshares - 1):
        t[i] = [random.randint(0, 2**k - 1) for _ in range(N)]
        ry = [(ry[j] ^ t[i][j]) for j in range(N)]
        tmp = bitslice(t[i], k)
        for j in range(k):
            y_bytes += int.to_bytes(tmp[j], byteorder="little", length=32)
    tmp = bitslice(ry, k)
    for j in range(k):
        y_bytes += int.to_bytes(tmp[j], byteorder="little", length=32)

    # Generate expected result.
    r = [(x[i] + y[i]) % q for i in range(N)]
    r = bitslice(r, k)
    r_bytes = bytes()
    for i in range(k):
    ...
    # Write input values.
    inputs = {'xb': x_bytes, 'yb': y_bytes, 'rb': rb_bytes}
...
if __name__ == '__main__':
    ...
    gen_secaddmodq_bc22_mlkem_test(
