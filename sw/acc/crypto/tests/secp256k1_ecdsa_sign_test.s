...
/**
 * Standalone secp256k1 ECDSA sign test
 *
 * Performs an ECDSA signing operation. An example message digest, the
 * private signing key and a random value k are provided in the .data section
 * below. The shares of k and d are randomized with a multiple of the curve
 * order before signing; this does not change the signature.
 *
...
start:
  ...
  /* call ECDSA signing subroutine in secp256k1 lib */
  jal      x1, secp256k1_sign
 ...
 *
 * Returns y = (x + r * n),
 *   where r is a 63-bit pseudorandom number and n is the secp256k1 curve
 *   order.
 *
...
/* second share of private key d (128 0s, then last 128 bits of d, 0s) */
.globl d1
.balign 32
d1:
  .zero 16
...
/* signature R output buffer */
.globl r
.balign 32
r:
  .zero 32

/* signature S output buffer */
.globl s
.balign 32
s:
  .zero 32
