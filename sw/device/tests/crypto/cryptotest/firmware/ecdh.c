...
enum {
  ...
  /**
   * Bytes in one share of an unmasked secp256k1 private key.
   */
  kSecp256k1PrivateKeyBytes = 32,
  /**
   * Bytes in one share of a masked secp256k1 private key.
   */
  kSecp256k1MaskedPrivateKeyBytes = 40,
  /**
   * Words in one share of a masked secp256k1 private key.
   */
  kSecp256k1MaskedPrivateKeyWords =
      kSecp256k1MaskedPrivateKeyBytes / sizeof(uint32_t),
  /**
   * Bytes in a secp256k1 public key coordinate.
   */
  kSecp256k1CoordinateBytes = 32,
  /**
   * Words in a secp256k1 public key coordinate.
   */
  kSecp256k1CoordinateWords = kSecp256k1CoordinateBytes / sizeof(uint32_t),
  /**
   * Bytes in one share of an ECDH/secp256k1 shared secret.
   */
  kSecp256k1SharedSecretBytes = 32,
};
...
/**
 * Run ECDH with curve secp256k1.
 *
 * The caller should ensure at least kSecp256k1SharedSecretBytes of space are
 * allocated at `ss`.
 *
 * If the cryptolib returns an error saying the input is invalid, the shared
 * secret will be zero and the `valid` output will be false.
 ...
 * @return Status code (OK or error).
 */
static status_t ecdh_secp256k1(cryptotest_ecdh_private_key_t d,
                               cryptotest_ecdh_coordinate_t qx,
                               cryptotest_ecdh_coordinate_t qy, uint32_t *ss,
                               bool *valid) {
  if (d.d0_len > kSecp256k1MaskedPrivateKeyBytes ||
      d.d1_len > kSecp256k1MaskedPrivateKeyBytes) {
    LOG_ERROR(
        "Bad secp256k1 private key share length (should both be <= %d): "
        "(d0 = %d, d1 = %d)",
        kSecp256k1PrivateKeyBytes, d.d0_len, d.d1_len);
    return INVALID_ARGUMENT();
  }
  if (qx.coordinate_len > kSecp256k1CoordinateBytes ||
      qy.coordinate_len > kSecp256k1CoordinateBytes) {
    LOG_ERROR(
        "Bad secp256k1 coordinate length (should both be <= %d): (x = %d, y = "
        "%d)",
        kSecp256k1CoordinateBytes, qx.coordinate_len, qy.coordinate_len);
    return INVALID_ARGUMENT();
  }

  // Construct the private key object.
  // TODO(#20762): once key-import exists for ECDH, use that instead.
  uint32_t private_keyblob[kSecp256k1MaskedPrivateKeyWords * 2];
  memset(private_keyblob, 0, sizeof(private_keyblob));
  memcpy(private_keyblob, d.d0, d.d0_len);
  memcpy(private_keyblob + kSecp256k1MaskedPrivateKeyWords, d.d1, d.d1_len);
  otcrypto_blinded_key_t private_key = {
      .config =
          {
              .version = kOtcryptoLibVersion1,
              .key_mode = kOtcryptoKeyModeEcdhSecp256k1,
              .key_length = kSecp256k1PrivateKeyBytes,
              .hw_backed = kHardenedBoolFalse,
              .exportable = kHardenedBoolTrue,
              .security_level = kOtcryptoKeySecurityLevelPassiveRemote,
          },
  ...
  // Construct the public key object.
  // TODO(#20762): once key-import exists for ECDH, use that instead.
  uint32_t public_key_buf[kSecp256k1CoordinateWords * 2];
  memset(public_key_buf, 0, sizeof(public_key_buf));
  memcpy(public_key_buf, qx.coordinate, qx.coordinate_len);
  memcpy(public_key_buf + kSecp256k1CoordinateWords, qy.coordinate,
         qy.coordinate_len);
  otcrypto_unblinded_key_t public_key = {
      .key_mode = kOtcryptoKeyModeEcdhSecp256k1,
      .key_length = sizeof(public_key_buf),
  ...
  // Create a destination for the shared secret.
  size_t shared_secret_words = kSecp256k1SharedSecretBytes / sizeof(uint32_t);
  uint32_t shared_secretblob[shared_secret_words * 2];
  memset(shared_secretblob, 0, sizeof(shared_secretblob));
  otcrypto_blinded_key_t shared_secret = {
      .config =
          {
              .version = kOtcryptoLibVersion1,
              .key_mode = kOtcryptoKeyModeAesCtr,
              .key_length = kSecp256k1SharedSecretBytes,
              .hw_backed = kHardenedBoolFalse,
              .exportable = kHardenedBoolTrue,
              .security_level = kOtcryptoKeySecurityLevelPassiveRemote,
          },
  ...
  switch (status.value) {
    ...
    case kOtcryptoStatusValueBadArgs: {
      // If the input was rejected (e.g. invalid public key, exit early.
      *valid = false;
      memset(ss, 0, kSecp256k1SharedSecretBytes);
      return OK_STATUS();
    }
    default: {
      TRY(status);
...
status_t handle_ecdh(ujson_t *uj) {
  ...
  switch (uj_curve) {
    ...
    case kCryptotestEcdhCurveSecp256k1: {
      uint32_t
          shared_secret_words[kSecp256k1SharedSecretBytes / sizeof(uint32_t)];
      TRY(ecdh_secp256k1(uj_private_key, uj_qx, uj_qy, shared_secret_words,
                         &valid));
      memcpy(uj_output.shared_secret, shared_secret_words,
             kSecp256k1SharedSecretBytes);
      uj_output.shared_secret_len = kSecp256k1SharedSecretBytes;
      break;
    }
    default:
