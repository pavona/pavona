...
status_t init_test_environment(void) {
  LOG_INFO(
      "Initializing modules entropy_src, csrng, edn0, edn1, aes, acc and "
      "alert_handler...");
...
status_t test_and_verify_acc_operation(void) {
  LOG_INFO("Starting ACC randomness test...(expect completion)");
  ...
  while (iter_cntr > 0) {
    ...
    // If ACC is no longer busy, it has completed successfully
    // Break the loop
    if (!acc_busy) {
      break;
    }
  ...
  // After timeout, if ACC is not busy, we shall conclude it's
  ...
  if (!acc_busy) {
    ...
    LOG_INFO("ACC program ran as expected with no hang");
    ...
    // If still busy after kIterateMaxRetries, ACC has not completed and failed
    LOG_ERROR("ACC program did not complete run");
    return INTERNAL();
  }
}
...
status_t execute_test(void) {
  ...
  for (uint16_t i = 0; i < ARRAYSIZE(window_sizes); i++) {
    ...
    // test the entropy (e.g. AES, ACC) to test EDN0
    // Step 7: Verify the entropy consuming endpoint(e.g. AES, ACC)
    // finishes its operation
    CHECK_STATUS_OK(test_and_verify_aes_operation());
    CHECK_STATUS_OK(test_and_verify_aes_operation());
    CHECK_STATUS_OK(test_and_verify_aes_operation());
    CHECK_STATUS_OK(test_and_verify_acc_operation());
    CHECK_STATUS_OK(test_and_verify_acc_operation());
    CHECK_STATUS_OK(test_and_verify_acc_operation());
  }
  LOG_INFO(
