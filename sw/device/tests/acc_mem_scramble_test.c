...
/**
 * Write values found at `word_addrs` to ACC memory at addresses `word_addrs`.
 *
 * @param ctx The acc context object.
 * @param num The number of addresses to write.
 * @param word_addrs The data to write and the word addresses to write to.
 * @param acc_write Pointer to the function to write the memory. It can be
...
/**
 * Check whether the contents at addresses `word_addrs` of an ACC memory match
 ...
 * @param acc_read Pointer to the function to read the memory. It can be
 *                  either `dif_acc_imem_read` or `dif_acc_dmem_read`.
...
bool test_main(void) {
  ...
  max = (uint32_t)dif_acc_get_imem_size_bytes(&acc) / sizeof(uint32_t) - 1;
  get_rand_words(&ibex, kNumAddrs, imem_offsets, max);
  max = (uint32_t)dif_acc_get_dmem_size_bytes(&acc) / sizeof(uint32_t) - 1;
  ...
  acc_write_mem_words(&acc, kNumAddrs, imem_offsets, dif_acc_imem_write);
  acc_write_mem_words(&acc, kNumAddrs, dmem_offsets, dif_acc_dmem_write);
