...
/**
 * OpenTitan program for ACC vertical side-channel analysis.
 *
 * This program implements the following simple serial commands:
 *   - Select the required ACC app (currently keygen/modinv) ('a')
 ...
 *
 * This handler has to be called to load a new app to acc.
 *
...
static void ecc256_app_select(const uint8_t *app_cmd, size_t app_cmd_len) {
  ...
  if (*app_cmd == 0) {
    // load keygen app
    SS_CHECK_STATUS_OK(acc_load_app(kAccAppP256KeyFromSeed));
  } else if (*app_cmd == 1) {
    // load mod inv app
    SS_CHECK_STATUS_OK(acc_load_app(kAccAppP256ModInv));
  } else {
...
static void simple_serial_main(void) {
  ...
  LOG_INFO("Load p256 keygen from seed app into ACC");
  SS_CHECK_STATUS_OK(acc_load_app(kAccAppP256KeyFromSeed));
