 ...
 *   sw/acc/crypto/test/p256_key_from_seed_test.s
 *
...
static void p256_ecdsa_gen_secret_key(const uint32_t *seed,
  ...
  // Read results.
  SS_CHECK_STATUS_OK(acc_dmem_read(kEcc256SeedNumWords, kAccVarD0, d0));
  SS_CHECK_STATUS_OK(acc_dmem_read(kEcc256SeedNumWords, kAccVarD1, d1));
}
...
static void p256_ecdsa_gen_keypair(const uint32_t *seed, const uint32_t *mask,
  ...
  SS_CHECK_STATUS_OK(acc_dmem_read(kEcc256SeedNumWords, kAccVarD0, d0));
  SS_CHECK_STATUS_OK(acc_dmem_read(kEcc256SeedNumWords, kAccVarD1, d1));
  SS_CHECK_STATUS_OK(acc_dmem_read(kEcc256CoordNumWords, kAccVarX, x));
  SS_CHECK_STATUS_OK(acc_dmem_read(kEcc256CoordNumWords, kAccVarY, y));
}
