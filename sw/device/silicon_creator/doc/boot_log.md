...
The boot process consists of three stages: `ROM`, `ROM_EXT`, and the first owner boot stage (e.g. `BL0`).
...
A top level system like Dragonfly stores this information in the retention SRAM as a `boot_log_t` struct to make it accessible to later boot stages.
