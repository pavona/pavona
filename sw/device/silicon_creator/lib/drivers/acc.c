...
static inline uint32_t acc_base(void) {
  return dt_acc_primary_reg_block(kDtAcc);
}
...
rom_error_t sc_acc_busy_wait_for_done(void) {
  ...
  } while (launder32(status) != kScAccStatusIdle &&
           launder32(status) != kScAccStatusLocked);
  res ^= ~status;
  ...
  return kErrorAccUnavailable;
}

/**
 * Helper function for writing to ACC's DMEM or IMEM.
 *
...
rom_error_t sc_acc_dmem_read(size_t num_words, sc_acc_addr_t src,
  ...
  for (; launder32(i) < num_words && launder32(r) < num_words; ++i, --r) {
    dest[i] = abs_mmio_read32(acc_base() + ACC_DMEM_REG_OFFSET + src +
                              i * sizeof(uint32_t));
  }
...
/**
 * Helper function for running an ACC command.
 *
 * This function blocks until ACC is idle.
 *
 * @param cmd ACC command.
...
static rom_error_t sc_acc_cmd_run(sc_acc_cmd_t cmd, rom_error_t error) {
  ...
  uint32_t reg = 0;
  do {
    reg = abs_mmio_read32(acc_base() + ACC_INTR_STATE_REG_OFFSET);
    res ^= (uint32_t)bitfield_bit32_read(reg, ACC_INTR_COMMON_DONE_BIT)
           << kResDoneBit;
  ...
  uint32_t err_bits = abs_mmio_read32(acc_base() + ACC_ERR_BITS_REG_OFFSET);
  res ^= err_bits;

  // Status should be kScAccStatusIdle; ACC can also issue a done interrupt
  // when transitioning to the "locked" state, so it is important to check
  // the status here.
  uint32_t status = abs_mmio_read32(acc_base() + ACC_STATUS_REG_OFFSET);

  if (launder32(res) == kErrorOk && launder32(err_bits) == 0 &&
      launder32(status) == kScAccStatusIdle) {
    HARDENED_CHECK_EQ(res, kErrorOk);
...
rom_error_t sc_acc_imem_sec_wipe(void) {
  return sc_acc_cmd_run(kScAccCmdSecWipeImem, kErrorAccSecWipeImemFailed);
}

rom_error_t sc_acc_dmem_sec_wipe(void) {
  return sc_acc_cmd_run(kScAccCmdSecWipeDmem, kErrorAccSecWipeDmemFailed);
}
 ...
 * @return OK if the addresses are valid, otherwise `kErrorAccInvalidArgument`.
 */
OT_WARN_UNUSED_RESULT
static rom_error_t check_app_address_ranges(const sc_acc_app_t app) {
...
rom_error_t sc_acc_load_app(const sc_acc_app_t app) {
  ...
  // Wipe memories.
  HARDENED_RETURN_IF_ERROR(sc_acc_dmem_sec_wipe());
  HARDENED_RETURN_IF_ERROR(sc_acc_imem_sec_wipe());
  ...
  sc_acc_addr_t imem_start_addr = 0;
  HARDENED_RETURN_IF_ERROR(
      sc_acc_imem_write(imem_num_words, app.imem_start, imem_start_addr));
