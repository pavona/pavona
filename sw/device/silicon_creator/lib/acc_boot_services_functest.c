...
rom_error_t sigverify_test(void) {
  ...
  RETURN_IF_ERROR(
      acc_boot_sigverify(&kEcdsaKey, &kEcdsaSignature, &digest, recovered_r));
...
rom_error_t attestation_advance_and_endorse_test(void) {
  ...
  uint32_t recovered_r[kEcdsaP256SignatureComponentWords];
  RETURN_IF_ERROR(acc_boot_sigverify(&pk, &sig, &digest, recovered_r));
...
// N.B. This test will lock ACC, so it needs to be the last test that runs.
...
bool test_main(void) {
  ...
  // Load the boot services ACC app.
