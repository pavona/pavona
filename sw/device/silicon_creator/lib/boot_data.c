...
static rom_error_t boot_data_default_get(lifecycle_state_t lc_state,
  ...
  switch (launder32(lc_state)) {
    ...
    case kLcStateProd:
      ...
      // Note: Default boot data is temporarily enabled in PROD/PROD_END to
      // support designs with no secure nonvolatile storage.
      //
      // However, it can be undesirable because it allows rolling back the
      // minimum security version data if the flash is wiped.  As a result,
      // this policy is expected to be changed when support for external flash
      // is added.
      res ^= kLcStateProd ^ kErrorBootDataNotFound ^ kErrorOk;
      break;
    case kLcStateProdEnd:
      HARDENED_CHECK_EQ(lc_state, kLcStateProdEnd);
      res ^= kLcStateProdEnd ^ kErrorBootDataNotFound ^ kErrorOk;
      break;
    case kLcStateRma:
