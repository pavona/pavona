...
#ifndef OPENTITAN_SW_DEVICE_SILICON_CREATOR_LIB_ACC_BOOT_SERVICES_H_
#define OPENTITAN_SW_DEVICE_SILICON_CREATOR_LIB_ACC_BOOT_SERVICES_H_
...
/**
 * Loads the ACC boot-services application.
 *
 * Loads the ACC program that runs attestation and code-signature
 * verification. The program can later be cleared by wiping ACC's IMEM and
 * DMEM, or by loading a diffierent ACC application.
 *
 * @return The result of the operation.
 */
OT_WARN_UNUSED_RESULT
rom_error_t acc_boot_app_load(void);
 ...
 *
 * This routine triggers the key manager to sideload key material into ACC,
 ...
 *
 * Expects the ACC boot-services program to already be loaded; see
 * `acc_boot_app_load`.
 *
...
/**
 * Wrapper for `acc_boot_attestation_keygen()` that generates an ECC P256
 * keypair to build a certificate around, using Keymgr and ACC, returning the
 ...
 *
 * This routine takes the same arguments as `acc_boot_attestation_keygen`, but
 * instead of computing the public key, it computes only the private key and
 * saves it to ACC's scratchpad memory.
 *
 * Expects the ACC boot-services program to already be loaded; see
 * `acc_boot_app_load`.
 *
 * @param additional_seed The attestation key generation seed to load.
 * @param key_type ACC attestation key type to generate. "DICE" attestation
...
/**
 * Clears any saved attestation key from ACC's scratchpad.
 *
 * This routine clears ACC's DMEM. If called after
 * `acc_boot_attestation_key_save`, it will clear the saved key.
 *
 * @return The result of the operation.
 */
OT_WARN_UNUSED_RESULT
 ...
 *
 * Must be called when there is a saved attestation key in ACC's scratchpad;
 * use `acc_boot_attestation_key_save` to store one.
 *
 ...
 * function.
 *
 * Note that the digest gets interpreted by ACC in little-endian order. If the
...
/**
 * Finish an ECDSA-P256 signature verify on ACC.
 *
...
#endif  // OPENTITAN_SW_DEVICE_SILICON_CREATOR_LIB_ACC_BOOT_SERVICES_H_
