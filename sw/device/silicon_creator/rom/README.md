...
The ROM contents are included in the wafer mask and programmed into the chip's metal ROM during wafer manufacturing.
As a result, the contents are fixed across an entire chip SKU and cannot be updated in-field.
...
In a dual-stage ROM, the second stage ROM (ROM1) supports optional patches loaded to OTP that are signed by the Silicon Creator and verified by the first-stage (ROM0).
