...
This guide will help you to setup an environment to run the ROM E2E tests on a device connected to an Ubuntu 22.04 host machine.
...
All the existing tests are documented in the [ROM E2E Testplan](../data/rom_e2e_testplan.hjson).
## Hardware setup
### FPGA
The E2E ROM tests run on the CW310 FPGA board with the following configuration:
 ...
 - A USB cable connecting the host computer to the USB-C Data (J8) connector which is connected to the SAM3U microcontroller on the CW310 FPGA board.
 ...
 - Optionally for future tests, a USB cable connecting the host computer to the USB-C J9 connector, which is connected to the `usbdev` IP block.

### JTAG adapter
Some tests will use the JTAG interface to interact with the device, as the CW310 doesn't have an embedded JTAG adapter, we can use an external adapter (Olimex ARM-USB-TINY-H JTAG) connected to the J13 header in the CW310.
...
- Clone the repository and install the software dependencies: [Get started](../../../../../doc/getting_started/README.md).
...
```
Once the hardware setup is done and the FPGA is connected to the USB, the full suite of ROM E2E tests can be executed with the command `./bazelisk.sh test` with the wildcard `sw/device/silicon_creator/rom/e2e`.
```sh
cd ${REPO_TOP}
