...
def strap_combination_test(name, env, value, evaluator = None, tags = [], extra_verilator_args = "", binaries = None):
...
def strap_combinations_test(name, env_firmware, tags = [], skip_value = []):
    all_tests = []
    for env, firmware in env_firmware.items():
