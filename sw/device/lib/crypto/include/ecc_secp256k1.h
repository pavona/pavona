...
#ifndef OPENTITAN_SW_DEVICE_LIB_CRYPTO_INCLUDE_ECC_SECP256K1_H_
#define OPENTITAN_SW_DEVICE_LIB_CRYPTO_INCLUDE_ECC_SECP256K1_H_

#include "datatypes.h"

/**
 * @file
 * @brief secp256k1 elliptic curve operations for OpenTitan cryptography
 * library.
 *
...
/**
 * Constructs an ECDSA/ECDH secp256k1 public key from affine coordinates.
 *
 ...
 * @return Result of the secp256k1 key construction.
 */
...
/**
 * Constructs an ECDSA/ECDH secp256k1 public key from affine coordinates,
 * performing an on-curve check on the constructed key.
 *
 * The caller should allocate space for the public key and set the `key` and
 * `key_length` fields accordingly.
 *
...
/**
 * Starts an asynchronous public key check for ECDSA/secp256k1.
 *
 * See `otcrypto_secp256k1_public_key_construct_and_check` for requirements on
...
/**
 * Finalizes an asynchronous public key check for ECDSA/secp256k1.
 *
 * See `otcrypto_secp256k1_public_key_construct_and_check` for requirements on
 * input and output values.
 *
...
/**
 * Deconstructs an ECDSA/ECDH secp256k1 public key into affine coordinates.
 *
 ...
 * @return Result of the secp256k1 key deconstruction.
 */
...
/**
 * Constructs an ECDSA/ECDH secp256k1 private key from scalar shares.
 *
 ...
 * @return Result of the secp256k1 private key construction.
 */
...
/**
 * Deconstructs an ECDSA/ECDH secp256k1 private key into scalar shares.
 *
 ...
 * @return Result of the secp256k1 private key deconstruction.
 */
...
/**
 * Generates a key pair for ECDSA with curve secp256k1.
 *
 ...
 * secp256k1 and the keyblob should be 80 bytes. The value in the `checksum`
...
/**
 * Generates an ECDSA signature with curve secp256k1.
 *
 ...
 *
 * Curve is fixed to secp256k1. The message digest must be exactly 256 bits and
...
/**
 * Generates an ECDSA signature with curve secp256k1 and verifies the signature
...
/**
 * Generates a key pair for ECDH with curve secp256k1.
 *
 * The caller should allocate and partially populate the blinded key struct,
 * including populating the key configuration and allocating space for the
 * keyblob. For a hardware-backed key, use the private key handle returned by
...
/**
 * Elliptic Curve Diffie Hellman shared secret generation with curve secp256k1.
 *
 * @param private_key Pointer to the blinded private key (d) struct.
 * @param public_key Pointer to the unblinded public key (Q) struct.
...
/**
 * Starts asynchronous key generation for ECDSA/secp256k1.
 *
 * See `otcrypto_ecdsa_secp256k1_keygen` for requirements on input values.
 *
...
/**
 * Finalizes asynchronous key generation for ECDSA/secp256k1.
 *
 * See `otcrypto_ecdsa_secp256k1_keygen` for requirements on input values.
 *
...
/**
 * Starts asynchronous signature generation for ECDSA/secp256k1.
 *
 * See `otcrypto_ecdsa_secp256k1_sign` for requirements on input values.
 *
 * @param private_key Pointer to the blinded private key (d) struct.
 * @param message_digest Message digest to be signed (pre-hashed).
 * @param[out] session_token Session token for this operation.
...
/**
 * Finalizes asynchronous signature generation for ECDSA/secp256k1.
 *
 * See `otcrypto_ecdsa_secp256k1_sign` for requirements on input values.
 *
 * May block until the operation is complete.
...
/**
 * Starts asynchronous signature verification for ECDSA/secp256k1.
 *
 * See `otcrypto_ecdsa_secp256k1_verify` for requirements on input values.
 *
 * @param public_key Pointer to the unblinded public key (Q) struct.
 * @param message_digest Message digest to be verified (pre-hashed).
 * @param signature Pointer to the signature to be verified.
...
/**
 * Finalizes asynchronous signature verification for ECDSA/secp256k1.
 *
 * See `otcrypto_ecdsa_secp256k1_verify` for requirements on input values.
 *
 * May block until the operation is complete.
...
/**
 * Starts asynchronous key generation for ECDH/secp256k1.
 *
 * See `otcrypto_ecdh_secp256k1_keygen` for requirements on input values.
 *
 * @param private_key Destination structure for private key, or key handle.
 * @param[out] session_token Session token for this operation.
...
/**
 * Finalizes asynchronous key generation for ECDH/secp256k1.
 *
 * See `otcrypto_ecdh_secp256k1_keygen` for requirements on input values.
 *
 * May block until the operation is complete.
...
/**
 * Starts asynchronous shared secret generation for ECDH/secp256k1.
 *
 * See `otcrypto_ecdh_secp256k1` for requirements on input values.
 *
 * @param private_key Pointer to the blinded private key (d) struct.
 * @param public_key Pointer to the unblinded public key (Q) struct.
 * @param[out] session_token Session token for this operation.
...
/**
 * Finalizes asynchronous shared secret generation for ECDH/secp256k1.
 *
 * See `otcrypto_ecdh_secp256k1` for requirements on input values.
 *
 * May block until the operation is complete.
...
#endif  // OPENTITAN_SW_DEVICE_LIB_CRYPTO_INCLUDE_ECC_SECP256K1_H_
