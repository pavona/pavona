...
void sysrst_ctrl_testutils_setup_dio(dif_pinmux_t *pinmux) {
  ...
  dif_result_t res =
      dif_pinmux_pad_write_attrs(pinmux, kTopEgretDirectPadsSysrstCtrlAonEcRstL,
  ...
  if (res == kDifError && out_attr.flags != kDifPinmuxPadAttrOpenDrain) {
    ...
    CHECK_DIF_OK(dif_pinmux_pad_write_attrs(
        pinmux, kTopEgretDirectPadsSysrstCtrlAonEcRstL, kDifPinmuxPadKindDio,
        in_attr, &out_attr));
    // Note: fallthrough with the modified `in_attr` so that the same attributes
    // are used for both pads.
  }
  CHECK_DIF_OK(dif_pinmux_pad_write_attrs(
      pinmux, kTopEgretDirectPadsSysrstCtrlAonFlashWpL, kDifPinmuxPadKindDio,
      in_attr, &out_attr));
}
