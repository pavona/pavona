...
pub fn read_cpu_crashdump_data(
    ...
    jtag.read_memory32(
        top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::CpuRegwen as u32,
        &mut buf,
    )?;
    ...
    jtag.read_memory32(
        top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::CpuInfoAttr as u32,
        &mut buf,
    )?;
    let len = buf[0];
    ...
    for i in 0..len {
        jtag.write_memory32(
            top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::CpuInfoCtrl as u32,
            &[RstmgrCpuInfoCtrl::INDEX.emplace(i) | RstmgrCpuInfoCtrl::EN],
        )?;
        jtag.read_memory32(
            top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::CpuInfo as u32,
            &mut buf,
        )?;
...
pub fn read_alert_crashdump_data(
    ...
    let mut buf = [0u32];
    jtag.read_memory32(
        top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::AlertInfoAttr as u32,
        &mut buf,
    )?;
    let len = buf[0];
    ...
    for i in 0..len {
        jtag.write_memory32(
            top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::AlertInfoCtrl as u32,
            &[RstmgrAlertInfoCtrl::INDEX.emplace(i) | RstmgrAlertInfoCtrl::EN],
        )?;
        jtag.read_memory32(
            top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::AlertInfo as u32,
            &mut buf,
        )?;
...
pub fn read_reset_reason(transport: &TransportWrapper, jtag_params: &JtagParams) -> Result<()> {
    ...
    let mut buf = [0u32];
    jtag.read_memory32(
        top::RSTMGR_AON_BASE_ADDR as u32 + RstmgrReg::ResetInfo as u32,
        &mut buf,
    )?;
