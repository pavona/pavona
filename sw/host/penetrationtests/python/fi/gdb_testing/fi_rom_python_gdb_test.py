    ...
    def test_rom_secure_boot(self):
        ...
        with open(campaign_file, "w") as campaign:
            ...
            try:
                ...
                # Tracing in done in two steps to jump over sc_acc_cmd_run which makes GDB hang
                ...
                # loading of the first app in ACC for efficiency purposes
                trace_start_address = rom_parser.get_inlined_function_address(
                    "sigverify_ecdsa_p256_key_id_get"
                )

                # We stop tracing when we execute the p256 verify in the acc
                trace_end_address = rom_parser.get_function_start_address("sc_acc_cmd_run")
                ...
                # We start from sc_acc_dmem_read which reads p256 verify's results from acc
                trace_start_address = rom_parser.get_function_start_address("sc_acc_dmem_read")
