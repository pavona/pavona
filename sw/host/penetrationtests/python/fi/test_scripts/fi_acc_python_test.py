...
from sw.host.penetrationtests.python.fi.host_scripts import fi_acc_functions
from sw.host.penetrationtests.python.fi.communication.fi_acc_commands import OTFIAcc
...
def load_test_data(test_name):
    data_path = r.Rlocation(
        "lowrisc_opentitan/sw/host/penetrationtests/python/fi/gold_responses/fi_acc.json"
    )
...
class AccFiTest(unittest.TestCase):

    def test_init(self):
        accfi = OTFIAcc(target)
        device_id, sensors, alerts, owner_page, boot_log, boot_measurements, version = (
            accfi.init(alert_config=common_library.default_fpga_friendly_alert_config)
        )
    ...
    def test_char_beq(self):
        actual_result = fi_acc_functions.char_beq(target, iterations)
    ...
    def test_char_bn_rshi(self):
        ...
        actual_result = fi_acc_functions.char_bn_rshi(target, iterations, data)
        actual_result_json = json.loads(actual_result)
    ...
    def test_char_mem(self):
        ...
        imem = False
        dmem = True
        actual_result = fi_acc_functions.char_mem(
    ...
    def test_char_pc(self):
        pc = 2224
        actual_result = fi_acc_functions.char_pc(target, iterations, pc)
        actual_result_json = json.loads(actual_result)
