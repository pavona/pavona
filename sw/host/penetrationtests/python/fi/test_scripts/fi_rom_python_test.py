...
from sw.host.penetrationtests.python.fi.host_scripts import fi_rom_functions
...
def load_test_data(test_name):
    data_path = r.Rlocation(
        "pavona_pavona/sw/host/penetrationtests/python/fi/gold_responses/fi_rom.json"
    )
...
class RomFiTest(unittest.TestCase):
    ...
    def test_char_rom_read(self):
        actual_result = fi_rom_functions.char_rom_read(target, iterations)
        actual_result_json = json.loads(actual_result)
        expected_result_json = load_test_data("char_rom_read")
